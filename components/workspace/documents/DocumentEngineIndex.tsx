...
function issueState(status: WorkspaceDocumentItem['dataStatus']) {
  return status === 'live-backed' ? 'Controlled workspace record' : 'Final issue blocked';
}
...
export default function DocumentEngineIndex() {
  return (
    <div className="mx-auto grid max-w-7xl gap-5 md:gap-6">
      ...
      <div className="hidden gap-5 md:grid lg:grid-cols-2 xl:grid-cols-3">
        {workspaceDocuments.map((document) => (
          <Link key={document.slug} href={`/workspace/documents/${document.slug}`} className="group rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-6 transition hover:-translate-y-0.5 hover:bg-white/[0.065]">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#b4975a]">{statusLabel(document.dataStatus)}</p>
            <h2 className="mt-3 text-xl font-semibold tracking-[-0.02em] text-white">{document.title}</h2>
            <p className="mt-3 min-h-24 text-sm leading-6 text-white/56">{document.description}</p>
            <p className="mt-5 text-sm font-semibold text-[#b4975a]">{document.dataStatus === 'live-backed' ? 'Open live-backed document' : 'Open protected preview'} ↗</p>
          </Link>
        ))}
      </div>
    </div>
