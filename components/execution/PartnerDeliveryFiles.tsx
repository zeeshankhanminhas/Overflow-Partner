...
export default function PartnerDeliveryFiles({token,onStateChange}:{token:string;onStateChange?:(state:{count:number;locked:boolean;cycle:number})=>void}){
  ...
  return <div className="grid gap-5 border-t border-black/10 pt-6">
    <div className="grid gap-2"><p className="text-xs font-medium uppercase tracking-[0.08em] text-neutral-500">Engineering output files · Execution cycle {cycle}</p><p className="text-sm leading-6 text-neutral-600">Attach the actual drawings, CAD/CAM outputs, models, programmes, BOMs or controlled output pack included in this delivery. A text manifest alone does not satisfy the delivery gate.</p></div>
    {!locked?<div><label className={`inline-flex min-h-11 items-center border border-black/20 px-5 py-2.5 text-sm font-medium transition ${busy?'cursor-not-allowed opacity-50':'cursor-pointer hover:border-black'}`}>{busy?'Uploading…':'Choose engineering output files'}<input ref={inputRef} className="sr-only" type="file" multiple accept={ACCEPT} onChange={uploadSelected} disabled={busy}/></label></div>:<div className="border-l-2 border-black/30 pl-4 text-sm text-neutral-600">This execution-cycle delivery has been submitted. Its attached files are now immutable.</div>}
    {loading?<p className="text-sm text-neutral-500">Loading delivery files…</p>:files.length?<div className="grid border-t border-black/10">{files.map(file=><div key={file.id} className="grid gap-3 border-b border-black/10 py-4 sm:grid-cols-[1fr_auto] sm:items-center"><div className="min-w-0"><p className="truncate text-sm font-medium">{file.original_filename}</p><p className="mt-1 text-xs text-neutral-500">{fileSize(Number(file.size_bytes||0))} · {file.submission_id?'Attached to submitted delivery':'Staged for this delivery'}</p></div>{!locked&&!file.submission_id?<button type="button" className="text-left text-xs font-medium uppercase tracking-[0.08em] text-neutral-600 hover:text-black sm:text-right" onClick={()=>removeFile(file)} disabled={busy}>Remove</button>:null}</div>)}</div>:<p className="text-sm text-neutral-500">No engineering output files attached to this execution cycle yet.</p>}
