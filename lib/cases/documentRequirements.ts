...
export function getCaseDocumentRequirements(input: {
  ...
  return {stageLabel,items:[
    ...
    {slug:'client-quote',enabled:quoteExists,requiredNow:quoteExists,minimumStatus:input.quoteStatus==='issued'?'issued':'approved',reason:input.quoteStatus==='issued'?'The controlled client quotation must remain at Issued state while the commercial decision is live.':'Sign and approve the controlled quotation before commercial issue to the client.',blockedReason:'Generate the client quote first.'},
