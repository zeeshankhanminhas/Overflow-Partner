...
function copy(template: string, payload: Record<string, unknown>) {
  ...
  const variants: Record<string, { heading: string; body: string; action: string }> = {
    enquiry_acknowledgement: {
      heading: 'Thank you for contacting Overflow Partner',
      body: `We have received your enquiry${company ? ` for ${company}` : ''}. We will review the requirement and confirm the next appropriate step.`,
      action: 'View enquiry',
    },
    technical_intake_invitation: {
      heading: 'A few technical details will help us assess the requirement',
      body: 'Please complete the secure technical intake so we can understand the scope, deliverables, files and timing before progressing.',
      action: 'Complete technical intake',
    },
    technical_intake_reminder: {
      heading: 'A polite reminder about your technical intake',
      body: `We are ready to review the requirement${reference ? ` under ${reference}` : ''}. The secure intake remains available whenever convenient${due ? ` and is due ${due}` : ''}.`,
      action: 'Continue technical intake',
    },
    partner_review_request: {
      heading: 'A controlled engineering review is ready',
      body: `Please review the technical package${reference ? ` for ${reference}` : ''} and submit feasibility, assumptions, risks, lead time and pricing through the secure review workspace.`,
      action: 'Open partner review',
    },
    partner_review_reminder: {
      heading: 'A polite reminder about the engineering review',
      body: `The controlled review remains open${due ? ` and is due ${due}` : ''}. Please submit the response through the secure workspace so the evidence remains linked to the case.`,
      action: 'Continue review',
    },
    quote_issued: {
      heading: 'Your controlled quotation is available',
      body: `The quotation${reference ? ` ${reference}` : ''} has been approved and issued for your review${due ? `. It remains valid until ${due}` : ''}.`,
    ...
    document_issued: {
      ...
      body: `A new controlled document${reference ? `, ${reference},` : ''} is available for review and download.`,
      action: 'Open document',
    },
    client_review_reminder: {
      heading: 'A polite reminder about the issued deliverable',
      body: `The controlled deliverable${reference ? ` ${reference}` : ''} is awaiting your review${due ? ` by ${due}` : ''}.`,
      action: 'Review deliverable',
    },
    nurture_capacity: {
      heading: 'A practical way to extend engineering capacity',
      body: 'Overflow Partner provides governed overflow support when internal teams are busy, without replacing the client’s engineering ownership or controls.',
      action: 'Discuss a requirement',
    },
    nurture_intake: {
      heading: 'What helps us assess an overflow requirement',
      body: 'A useful first assessment normally needs the intended deliverables, source files, discipline, timescale, standards, review expectations and any known constraints.',
      action: 'Share a requirement',
    },
    nurture_process: {
      heading: 'From enquiry to controlled delivery',
      body: 'Each requirement moves through technical intake, partner assessment, commercial review, controlled quotation, Project 360 delivery, internal review and controlled issue.',
      action: 'See how it works',
    },
    nurture_checkin: {
      heading: 'Planning for upcoming engineering capacity',
      body: 'Should an upcoming project create a temporary CAD, CAM or engineering-documentation bottleneck, the requirement can be assessed before the pressure becomes urgent.',
      action: 'Start a conversation',
    },
    nurture_final: {
      heading: 'A final check-in from Overflow Partner',
      body: 'We will leave things here for now. When overflow capacity is useful, you can return to the enquiry at any time or reply directly to this message.',
      action: 'Return to your enquiry',
    },
    system_failure: {
      heading: 'Overflow Partner notification requires attention',
      body: escapeHtml(payload.message || 'A notification could not be delivered after repeated attempts.'),
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f3f1ec;padding:32px 12px"><tr><td align="center">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px;background:#fff;border:1px solid #d9d5cc">
        <tr><td style="padding:28px 32px 18px;border-top:5px solid #e95d2a">
          ...
          <div style="margin-top:6px;font-size:13px;color:#6f6a61">Governed engineering capacity</div>
        </td></tr>
