...
function normaliseDocumentStatus(status: unknown): GovernedDocumentStatus {
...
export function resolveEvidenceState(documents: WorkspaceDocument[], requirements: EvidenceRequirement[]): ResolvedEvidenceState[] {
  return requirements.map(requirement => {
    ...
    const satisfied = Boolean(document) && documentStatusRank[currentStatus] >= documentStatusRank[requirement.requiredStatus];
