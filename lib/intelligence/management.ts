...
export async function getManagementIntelligence(supabase: any, organisationId: string, period: ReportingPeriod = '90d') {
  ...
  const [
    prospectsResult, leadsResult, quotesResult, projectsResult,
    deliveryResult, invoicesResult, paymentsResult, payablesResult, partnerPaymentsResult,
    documentsResult, issueRecordsResult, partnersResult, partnerReviewsResult, risksResult,
    exceptions,
  ] = await Promise.all([
  ...
  const quotes = (quotesResult.data || []) as any[];
  ...
  const payables = (payablesResult.data || []) as any[];
  const partnerPayments = (partnerPaymentsResult.data || []) as any[];
  const documents = (documentsResult.data || []) as any[];
  const issues = (issueRecordsResult.data || []) as any[];
  const partners = (partnersResult.data || []) as any[];
  const partnerReviews = (partnerReviewsResult.data || []) as any[];
  const risks = (risksResult.data || []) as any[];
  ...
  const wonLeads = periodLeads.filter(row => row.status === 'won');
  const lostLeads = periodLeads.filter(row => row.status === 'lost');
  ...
  const openQuoteValue = sum(openQuotes, row => Number(row.total || 0));
  const acceptedValue = sum(acceptedQuotes, row => Number(row.total || 0));
  ...
  const overdueDelivery = openDelivery.filter(row => row.due_date && new Date(row.due_date).getTime() < now);
  const blockedDelivery = openDelivery.filter(row => row.status === 'blocked');
  ...
  const receivables = sum(invoiceStates.filter(row => !row.state.settled), row => row.state.balance);
  const overdueReceivables = sum(invoiceStates.filter(row => row.state.overdue && !row.state.settled), row => row.state.balance);
  const partnerCommitted = sum(payables.filter(row => !['draft','cancelled'].includes(row.status)), row => Number(row.total || 0));
  const partnerOutstanding = sum(payableStates.filter(row => !row.state.settled), row => row.state.balance);
  const partnerCashPaid = sum(partnerPayments.filter(row => row.status === 'cleared'), row => Number(row.amount || 0));
  const forecastGrossMargin = invoicedValue - partnerCommitted;
  const realisedCashContribution = cashCollected - partnerCashPaid;

  const currentDocuments = documents.filter(row => row.is_current_revision !== false);
  const periodDocuments = documents.filter(row => inWindow(row.created_at, since));
  const periodIssues = issues.filter(row => inWindow(row.issued_at, since));
  const waitingDocuments = currentDocuments.filter(row => ['in_review','changes_requested','signed','approved'].includes(String(row.status)));
  const issuedDocuments = periodDocuments.filter(row => ['issued','published'].includes(String(row.status)));
  const approvalCycleHours = documents.map(row => hoursBetween(row.created_at, row.approved_at)).filter((v): v is number => v !== null);
  const issueCycleHours = documents.map(row => hoursBetween(row.created_at, row.issued_at)).filter((v): v is number => v !== null);

  const partnerMap = new Map(partners.map(row => [row.id, row]));
  const partnerStats = new Map<string, { id:string; name:string; requests:number; submitted:number; onTime:number; responseHours:number[]; rating:number|null }>();
  for (const review of partnerReviews.filter(row => inWindow(row.sent_at || row.created_at, since))) {
    const partner = partnerMap.get(review.partner_id) as any;
    if (!partner) continue;
    const stat = partnerStats.get(partner.id) || { id:partner.id, name:partner.company_name, requests:0, submitted:0, onTime:0, responseHours:[], rating:partner.rating == null ? null : Number(partner.rating) };
    stat.requests += 1;
    if (review.submitted_at) {
      stat.submitted += 1;
      if (!review.response_due_at || new Date(review.submitted_at).getTime() <= new Date(review.response_due_at).getTime()) stat.onTime += 1;
      const h = hoursBetween(review.sent_at || review.created_at, review.submitted_at);
      if (h !== null) stat.responseHours.push(h);
    }
    partnerStats.set(partner.id, stat);
  }
  const partnerPerformance = [...partnerStats.values()].map(stat => ({
    ...stat,
    responseRate: ratio(stat.submitted, stat.requests),
    onTimeRate: ratio(stat.onTime, stat.submitted),
    averageResponseHours: stat.responseHours.length ? sum(stat.responseHours, value => value) / stat.responseHours.length : null,
  })).sort((a,b) => b.responseRate - a.responseRate || b.onTimeRate - a.onTimeRate || (a.averageResponseHours ?? 99999) - (b.averageResponseHours ?? 99999));

  const workloadMap = new Map<string, { owner:string; open:number; overdue:number; blocked:number; review:number }>();
  for (const item of openDelivery) {
    const owner = item.owner?.full_name || item.owner?.first_name || 'Unassigned';
    const stat = workloadMap.get(owner) || { owner, open:0, overdue:0, blocked:0, review:0 };
    stat.open += 1;
    if (item.due_date && new Date(item.due_date).getTime() < now) stat.overdue += 1;
    if (item.status === 'blocked') stat.blocked += 1;
    if (['internal_review','client_review','ready_to_issue'].includes(item.status)) stat.review += 1;
    workloadMap.set(owner, stat);
  }
  ...
  for (const row of periodProspects) {
    const source = row.source || 'unknown';
    const stat = sourceMap.get(source) || { source, prospects:0, converted:0 };
    stat.prospects += 1;
    if (row.status === 'converted' || row.converted_lead_id) stat.converted += 1;
  ...
  const exceptionCategories = ['delivery','finance','document','task','communication'].map(category => ({ category, count:exceptions.filter(item => item.category === category).length }));
  const openRisks = risks.filter(row => row.status !== 'closed');
  const highRisks = openRisks.filter(row => Number(row.likelihood||0) * Number(row.impact||0) >= 12);

  return {
    period,
    since,
    acquisition: {
      prospects: periodProspects.length,
      convertedProspects: convertedProspects.length,
      prospectConversionRate: ratio(convertedProspects.length, periodProspects.length),
      cases: periodLeads.length,
      wonCases: wonLeads.length,
      lostCases: lostLeads.length,
      quoteIssued: periodQuotes.length,
      quoteAccepted: acceptedQuotes.length,
      quoteWinRate: ratio(acceptedQuotes.length, decidedQuotes.length),
      openQuoteValue,
      acceptedValue,
      sourcePerformance,
    },
    delivery: {
      ...
      openItems: openDelivery.length,
      overdueItems: overdueDelivery.length,
      blockedItems: blockedDelivery.length,
      reviewItems: reviewDelivery.length,
      completedItems: periodCompletedDelivery.length,
      onTimeRate: ratio(onTimeDelivery.length, periodDueDelivery.length || periodCompletedDelivery.length),
      workload,
    },
    finance: {
      invoicedValue,
      ...
      receivables,
      overdueReceivables,
      collectionRate: ratio(cashCollected, invoicedValue),
      partnerCommitted,
      partnerOutstanding,
      partnerCashPaid,
      forecastGrossMargin,
      forecastMarginRate: ratio(forecastGrossMargin, invoicedValue),
      realisedCashContribution,
    },
    documents: {
      created: periodDocuments.length,
      issued: issuedDocuments.length,
      issueRecords: periodIssues.length,
      waiting: waitingDocuments.length,
      issueRate: ratio(issuedDocuments.length, periodDocuments.length),
      averageApprovalHours: approvalCycleHours.length ? sum(approvalCycleHours, value=>value)/approvalCycleHours.length : null,
      averageIssueHours: issueCycleHours.length ? sum(issueCycleHours, value=>value)/issueCycleHours.length : null,
    },
    partners: {
      approved: partners.filter(row => row.status === 'approved').length,
      performance: partnerPerformance,
    },
    exceptions: {
      summary: exceptionSummary,
      categories: exceptionCategories,
      top: exceptions.slice(0,8),
    },
    risk: {
