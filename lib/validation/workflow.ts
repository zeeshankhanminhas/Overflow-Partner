...
export const partnerSchema = z.object({
  ...
  services: z.string().trim().min(2).max(1000),
  contact_name: optionalText,
  email: z.string().trim().email().optional().or(z.literal('')),
  phone: optionalText,
  nda_signed: z.enum(['true', 'false']).default('false'),
  status: z.enum(['prospective', 'approved', 'suspended', 'inactive']).default('prospective'),
...
export const partnerQuoteSchema = z.object({
  ...
  lead_time_days: z.coerce.number().int().min(0).optional().or(z.literal('')),
  valid_until: optionalDate,
  status: z.enum(['requested', 'received', 'under_review', 'selected', 'declined', 'expired']).default('requested'),
  notes: optionalText,
});

export const commercialReviewSchema = z.object({
  lead_id: z.string().uuid(),
  partner_quote_id: z.string().uuid().optional().or(z.literal('')),
...
export const clientQuoteSchema = z.object({
  lead_id: z.string().uuid(),
  commercial_review_id: z.string().uuid().optional().or(z.literal('')),
  quote_number: z.string().trim().min(3).max(80),
  revision: z.coerce.number().int().min(1).default(1),
  subtotal: z.coerce.number().min(0),
  vat: z.coerce.number().min(0).default(0),
  currency: z.string().trim().length(3).default('GBP'),
  valid_until: optionalDate,
  status: z.enum(['draft', 'internal_review', 'issued', 'accepted', 'declined', 'expired', 'superseded']).default('draft'),
});

export const projectSchema = z.object({
  lead_id: z.string().uuid(),
  quote_id: z.string().uuid().optional().or(z.literal('')),
  project_number: z.string().trim().min(3).max(80),
  title: z.string().trim().min(3).max(220),
  status: z.enum(['planning', 'active', 'waiting', 'review', 'completed', 'closed', 'cancelled']).default('planning'),
