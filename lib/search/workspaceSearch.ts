...
export type WorkspaceSearchResult={entity_type:string;entity_id:string;title:string;subtitle:string|null;href:string};
export function normaliseWorkspaceSearchQuery(value:string){return value.replace(/[,()%]/g,' ').replace(/\s+/g,' ').trim();}

export async function searchWorkspace(supabase:SupabaseClient,organisationId:string,rawQuery:string,limit=30):Promise<WorkspaceSearchResult[]>{
  const q=normaliseWorkspaceSearchQuery(rawQuery);if(q.length<2)return [];
  const pattern=`%${q}%`;
  ...
  const errors=[companies.error,projects.error,partners.error,cases.error,quotes.error,documents.error,enquiries.error].filter(Boolean);if(errors.length)throw new Error(errors[0]?.message||'Search unavailable.');
  ...
  const needle=q.toLowerCase();results.sort((a,b)=>{const aExact=a.title.toLowerCase().startsWith(needle)?0:1;const bExact=b.title.toLowerCase().startsWith(needle)?0:1;return aExact-bExact||a.title.localeCompare(b.title);});return results.slice(0,limit);
}
