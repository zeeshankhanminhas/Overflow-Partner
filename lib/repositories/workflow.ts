...
export async function listCommercialReviews(supabase: SupabaseClient, organisationId: string) { const { data, error } = await supabase.from('commercial_reviews').select('*').eq('organisation_id', organisationId).order('created_at', { ascending: false }).limit(100); if (error) throw new Error(error.message); return (data ?? []) as CommercialReview[]; }
...
export async function listProjects(supabase: SupabaseClient, organisationId: string) { const {data,error}=await supabase.from('projects').select('*').eq('organisation_id',organisationId).order('created_at',{ascending:false}).limit(100); if(error)throw new Error(error.message); return (data??[]) as Project[]; }
...
export async function listTasks(supabase: SupabaseClient, organisationId: string) { const {data,error}=await supabase.from('tasks').select('*').eq('organisation_id',organisationId).order('created_at',{ascending:false}).limit(100); if(error)throw new Error(error.message); return (data??[]) as Task[]; }
...
export async function listProfiles(supabase: SupabaseClient, organisationId: string) { const {data,error}=await supabase.from('profiles').select('id,organisation_id,full_name,first_name,last_name,email,role,is_active,developer_delete_enabled,created_at').eq('organisation_id',organisationId).order('created_at').limit(100); if(error)throw new Error(error.message); return (data??[]) as Profile[]; }
