...
export async function createProspectPartnerReviewAction(formData: FormData) {
  ...
  try {
    ...
    const { data: request, error } = await supabase.rpc('op_create_prospect_partner_review_request', {
      ...
      p_response_due_at: due.toISOString(),
    ...
    if (partner.email) {
      const payload = {
        ...
        project: `${prospect.company_name} — ${prospect.project_type || 'Engineering requirement'}`,
        reference: request?.case_reference || requestId,
