...
export async function convertProspectAction(formData: FormData): Promise<ActionResult<Lead>> {
  try {
    ...
    if (prospect.status !== 'qualified') return { ok: false, error: 'Only qualified prospects can be converted into leads.' };
    if (prospect.converted_lead_id) return { ok: false, error: 'This prospect has already been converted.' };
    const parsed = leadInputSchema.safeParse({ company_id: prospect.company_id || '', contact_id: prospect.contact_id || '', company_name: prospect.company_name, contact_name: prospect.contact_name || '', contact_email: prospect.email || '', project_type: String(formData.get('project_type') || ''), title: String(formData.get('title') || `${prospect.company_name} engineering requirement`), service: String(formData.get('service') || 'Engineering overflow support'), priority: String(formData.get('priority') || 'normal'), status: 'new', source: prospect.source, prospect_id: prospect.id, notes: prospect.notes || '' });
    if (!parsed.success) return { ok: false, error: 'The prospect could not be converted because required lead information is missing.', fieldErrors: parsed.error.flatten().fieldErrors };
    ...
    await recordActivity(supabase, { organisationId, entityType: 'prospect', entityId: prospect.id, userId: user.id, eventType: 'prospect.converted', oldValue: prospect, newValue: convertedProspect, eventData: { lead_id: lead.id } });
    await recordActivity(supabase, { organisationId, entityType: 'lead', entityId: lead.id, userId: user.id, eventType: 'lead.created_from_prospect', newValue: lead, eventData: { prospect_id: prospect.id } });
    revalidatePath('/workspace/acquisition'); revalidatePath('/workspace/leads'); if (prospect.company_id) revalidatePath(`/workspace/companies/${prospect.company_id}`); revalidatePath('/workspace');
