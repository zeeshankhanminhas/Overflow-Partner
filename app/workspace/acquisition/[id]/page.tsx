...
export default async function AcquisitionRecordPage({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<Record<string,string|string[]|undefined>>}){
  ...
  const nextActionPanel=<div><p className="vp-kicker">Next permitted action</p><h2 style={{marginTop:6}}>{acquisition.nextAction}</h2><p>{acquisition.nextReason}</p>
    ...
    {acquisition.actionKey==='wait_customer'?<div className="vp-callout"><strong>Waiting on customer</strong><p>The Step 2 invitation is live. No internal decision is required until the customer submits it.</p>{canUseDeveloperLink?<form action={generateStep2TestLinkFormAction} style={{marginTop:12}}><input type="hidden" name="prospect_id" value={id}/><button className="button secondary">Generate customer Step 2 test link</button></form>:null}</div>:null}
    ...
    {acquisition.actionKey==='closed'?<div className="vp-callout"><strong>Acquisition closed</strong><p>This opportunity has a governed No-Go outcome. It cannot progress to Case 360.</p></div>:null}
  </div>;
  ...
  const evidence=<div style={{display:'grid',gap:20}}>{submission?<AcquisitionTechnicalBrief submission={submission}/>:<div className="vp-empty">No technical submission exists yet.</div>}{intakeFiles.length?<div><p className="vp-kicker">Customer source evidence</p>{intakeFiles.map((file:any)=><div key={file.id} className="project-os-history-row"><span>↳</span><div><strong>{file.original_filename}</strong><small>{Math.max(1,Math.round(Number(file.size_bytes||0)/1024))} KB · {dateTime(file.uploaded_at)}</small></div></div>)}</div>:null}{response?<div className="vp-callout"><strong>Partner evidence received</strong><p>{response.reviewer_name}{response.reviewer_role?` · ${response.reviewer_role}`:''} · {String(response.feasibility).replaceAll('_',' ')} · {response.confidence_percent}% confidence.</p></div>:null}</div>;
  const activities=<div className="vp-facts"><Fact label="Intake sent" value={dateTime(session?.sent_at)}/><Fact label="Customer submitted" value={dateTime(session?.submitted_at)}/><Fact label="Partner request" value={dateTime(partnerRequest?.sent_at)}/><Fact label="Partner responded" value={dateTime(partnerRequest?.submitted_at)}/><Fact label="Partner response due" value={dateTime(partnerRequest?.response_due_at)}/><Fact label="Next movement" value={prospect.next_action}/></div>;
  const communications=<div><p style={{color:'var(--op-muted)',lineHeight:1.6}}>Customer intake and partner review remain owned by Acquisition until the Go / No-Go decision permits Case 360 creation.</p>{prospect.company_id?<Link className="button secondary" href={`/workspace/companies/${prospect.company_id}`}>Open company record</Link>:null}</div>;
  const history=<div>{events.length===0?<p>No prospect audit events recorded yet.</p>:events.slice(0,16).map((event:any)=><div className="project-os-history-row" key={event.id}><span>→</span><div><strong>{String(event.event_type).replaceAll('_',' ').replaceAll('.',' ')}</strong><small>{dateTime(event.created_at)}</small></div></div>)}</div>;
  const metadata=<div className="vp-facts"><Fact label="Prospect ID" value={prospect.id}/><Fact label="Created" value={dateTime(prospect.created_at)}/><Fact label="Updated" value={dateTime(prospect.updated_at)}/><Fact label="Partner review ref" value={partnerRequest?.case_reference}/><Fact label="Partner price" value={partnerQuote?`${partnerQuote.currency} ${Number(partnerQuote.price).toFixed(2)}`:'Not received'}/></div>;
  const audit=<p style={{color:'var(--op-muted)',lineHeight:1.6}}>The customer submission, partner response, partner price, internal Go / No-Go and Case conversion are separate governed events. Case 360 cannot become the active owner until all Acquisition gates are satisfied.</p>;
