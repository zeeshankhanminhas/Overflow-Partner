...
export default async function AcquisitionRecordPage({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<Record<string,string|string[]|undefined>>}){
  ...
  let currentState='Prospect captured',nextAction='Create Step 2 intake',nextReason='Structured technical evidence is required before this opportunity can be qualified.';
  if(session&&!submitted){currentState='Awaiting customer technical intake';nextAction='Wait for customer submission';nextReason='The Step 2 intake has been issued and remains with the customer.';}
