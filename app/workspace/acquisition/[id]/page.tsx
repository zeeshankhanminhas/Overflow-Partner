...
export default async function AcquisitionRecordPage({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<Record<string,string|string[]|undefined>>}){
  ...
  const communications=<div><p style={{color:'var(--op-muted)',lineHeight:1.6}}>Client requirements and delivery-partner review stay attached to this opportunity until the commercial decision is complete.</p>{prospect.company_id?<Link className="button secondary" href={`/workspace/companies/${prospect.company_id}`}>Open client record</Link>:null}</div>;
