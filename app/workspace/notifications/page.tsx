...
export default async function NotificationsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <section className="vp-page">
    <ProductPageHeader eyebrow="Admin · Notifications" title="Notification delivery" description="Monitor automated message delivery health. Business correspondence belongs in Messages; operational exceptions belong in Issues." actions={<><Link className="button secondary" href="/workspace/communications">Messages</Link><Link className="button secondary" href="/workspace/exceptions">Issues</Link></>} />
    <ProductMetrics label="Delivery health">
      <ProductMetric label="Sent" value={sent} detail="Delivered in this view" tone={sent?'complete':'neutral'} />
      <ProductMetric label="Queued" value={pending} detail="Pending or processing" tone={pending?'waiting':'neutral'} />
      <ProductMetric label="Failed" value={failed} detail="Needs delivery intervention" tone={failed?'blocked':'complete'} />
      <ProductMetric label="Cancelled" value={cancelled} detail="Stopped before delivery" />
    </ProductMetrics>

    <ProductFilterBar>
      {['all','pending','processing','sent','failed','cancelled'].map(value=><Link key={value} className={`button ${status===value?'':'secondary'}`} href={`/workspace/notifications?status=${value}&category=${category}`}>{humanise(value)}</Link>)}
      <form method="get" className="product-toolbar__group" style={{marginLeft:'auto'}}>
        {status!=='all'?<input type="hidden" name="status" value={status}/>:null}
        <select name="category" defaultValue={category}><option value="all">All categories</option><option value="transactional">Transactional</option><option value="reminder">Reminder</option><option value="nurture">Nurture</option><option value="system">System</option></select>
    ...
    <section>
      ...
      {rows.length===0?<ProductEmptyState title="No notifications match this view" description="Change the delivery status or category filter to broaden the register." />:<ProductRegister>
        {rows.map(row=>{const href=relatedHref(row.entity_type,row.entity_id);return <ProductRegisterRow key={row.id} href={href}>
          <div><strong>{row.subject}</strong><p>{row.recipient_name||row.recipient_email} · {humanise(row.category)}</p>{row.last_error?<small>{row.last_error}</small>:null}</div>
          <ProductStatus tone={tone(row.status)}>{humanise(row.status)}</ProductStatus>
          ...
          <div><small>Attempts</small><strong style={{display:'block',marginTop:3}}>{row.attempts} / {row.max_attempts}</strong></div>
          <strong>{href?'Open related record →':'Delivery record'}</strong>
        </ProductRegisterRow>})}
      </ProductRegister>}
    </section>
  </section>;
