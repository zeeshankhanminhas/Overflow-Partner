...
export default async function Page({searchParams}:{searchParams?:Promise<Record<string,string|undefined>>}){
  ...
  return <section className="vp-page">
    <header className="vp-header">
      <div><p className="vp-kicker">Projects</p><h1>Accepted work in delivery.</h1><p className="vp-subtitle">Projects are created from accepted client quotes. Open a project to continue delivery; this register is not the operating surface.</p></div>
    ...
    <section><div className="vp-section-title"><div><p className="vp-label">Primary object</p><h2>Delivery records</h2></div></div><div className="vp-list">{projects.length===0?<div className="vp-empty">No accepted work has entered delivery.</div>:projects.map(p=><Link href={`/workspace/projects/${p.id}`} className="vp-row" key={p.id}><div><h3>{p.project_number} · {p.title}</h3><p>{p.start_date||'Start not set'} → {p.due_date||'Due date not set'}</p></div><div className="vp-row-status">{workspaceLabel(p.status,'project')}</div><div><strong>Open delivery →</strong></div></Link>)}</div></section>
  </section>;
}
