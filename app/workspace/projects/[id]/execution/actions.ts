...
export async function saveExecutionAssignmentAction(formData: FormData) {
  ...
  try {
    ...
    if(scopeError||!scope||scope.project_id!==projectId||!['scope-of-work','statement-of-work'].includes(String(scope.document_type))||!['approved','issued','published'].includes(String(scope.status))||!scope.is_current_revision) {
      throw new Error('Select the current approved Scope of Work or Statement of Work as the controlled execution scope.');
    }

    const payload={
      organisation_id:organisationId,project_id:projectId,partner_id:partnerId,scope_document_id:scopeDocumentId,
      partner_contact_name:String(formData.get('partner_contact_name')||'').trim()||null,partner_contact_email:contactEmail,
      planned_start_date:String(formData.get('planned_start_date')||'')||null,committed_due_date:String(formData.get('committed_due_date')||'')||project.due_date||null,
      reporting_cadence:cadence,release_notes:String(formData.get('release_notes')||'').trim()||null,created_by:user.id,
    };
