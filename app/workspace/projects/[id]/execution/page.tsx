...
export default async function PartnerExecutionControlPage({ params, searchParams }: {
  ...
  return <section className="stack" style={{gap:24,maxWidth:1180}}>
    ...
    {query.execution_link?<div className="card stack" style={{gap:10}}><p className="eyebrow">Secure Partner link</p><strong>Partner workspace ready.</strong><input readOnly value={query.execution_link} onFocus={(event)=>event.currentTarget.select()}/><small>This active link remains available below while the session is valid. You do not need to rotate it just to reopen the Partner workspace.</small></div>:null}
