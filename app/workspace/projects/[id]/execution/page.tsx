...
function safeExecutionLink(baseUrl:string, sessionId:string){
  try{return `${baseUrl}/execution/${createExecutionSessionToken(sessionId)}`;}catch{return null;}
}
...
export default async function PartnerExecutionControlPage({ params, searchParams }: {
  ...
  const currentExecutionLink=sessionUsable?safeExecutionLink(baseUrl,session.id):null;
  const signingUnavailable=Boolean(sessionUsable&&!currentExecutionLink);
  ...
  }else if(signingUnavailable){
    presentation={state:'Partner access configuration required',headline:'Partner access configuration required',summary:'The governed execution session exists, but this environment cannot sign a secure Partner link. Execution evidence remains intact.',tone:'attention',waitingOn:{actor:'system',label:'Workspace configuration'},nextAction:{label:'Configure execution signing',available:false,kind:'wait',reason:'Set execution-link signing credentials for this environment before issuing or replacing Partner access.'},blockers:['Execution-link signing credentials are not configured for this environment.'],warnings:[],completed:['Partner release recorded'],primaryActions:[]};
  ...
  const notices=<>{query.success?<div className="project-os-notice success"><strong>{query.success}</strong></div>:null}{query.error?<div className="project-os-notice error"><strong>{query.error}</strong></div>:null}{signingUnavailable?<div className="project-os-notice error"><strong>Secure Partner access is unavailable in this environment.</strong><p>The execution record is safe. Configure execution-link signing credentials before attempting to issue or replace Partner access.</p></div>:null}</>;
  ...
  else if(signingUnavailable)nextAction=<p style={{color:'var(--op-muted)'}}>No execution state has been changed. Configure execution-link signing for this Preview environment, then return here.</p>;
  ...
  const readiness=releaseIssues.length||signingUnavailable?<div className="op-evidence-list">{releaseIssues.map(issue=><EvidenceRow key={issue} label="Unresolved" value={issue} tone="attention"/>)}{signingUnavailable?<EvidenceRow label="System configuration" value="Execution-link signing credentials required" tone="attention"/>:null}</div>:undefined;
  ...
  const accessContext=assignment?<div className="stack" style={{gap:14}}><EvidenceRow label="Release status" value={sessionUsable?'Released':'Access not active'} tone={sessionUsable?'complete':'waiting'}/><EvidenceRow label="Recipient" value={assignment.partner_contact_email||commercialPartner?.email||'Not recorded'}/><EvidenceRow label="Scope" value={assignedScope?`${assignedScope.reference} · ${assignedScope.title}`:'Controlled scope recorded'}/><EvidenceRow label="Committed due" value={formatDate(assignment.committed_due_date||project.due_date)}/>{currentExecutionLink?<><a className="button" href={currentExecutionLink} target="_blank" rel="noreferrer">Open Partner workspace</a><ProductDisclosure summary="Replace secure Partner link"><form action={guardedGenerateExecutionLinkAction} className="stack" style={{paddingTop:12}}><input type="hidden" name="project_id" value={id}/><p>Replacing the link revokes the current secure access. Use this only for an access problem.</p><button className="button secondary" type="submit">Replace Partner link</button></form></ProductDisclosure></>:signingUnavailable?<div className="vp-callout"><strong>Secure Partner link unavailable</strong><p>This environment is missing execution-link signing credentials. The existing execution assignment and session have not been changed.</p></div>:null}</div>:undefined;

  const history=<div className="stack" style={{gap:10}}>{progress.slice(0,6).map(item=><EvidenceRow key={item.id} label={formatDate(item.submitted_at)} value={stateLabel(item.progress_state)} meta={item.percent_complete===null||item.percent_complete===undefined?undefined:`${item.percent_complete}%`}/>)}{progress.length===0?<p>No Partner progress history yet.</p>:null}</div>;
  const audit=<div className="stack" style={{gap:10}}><p>Execution assignment, secure-session, commencement, progress, exception and delivery records remain governed database evidence. Missing Preview signing configuration does not alter them.</p><Link href={`/workspace/activity`}>Open activity audit →</Link></div>;

  return <RecordWorkspace header={header} notices={notices} presentation={presentation} readiness={readiness} nextAction={nextAction} summary={summary} evidence={evidence} activities={activities} metadata={accessContext} history={history} audit={audit}/>;
}
