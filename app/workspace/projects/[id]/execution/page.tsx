...
export default async function PartnerExecutionControlPage({ params, searchParams }: {
  ...
  const latestProgress=progress[0];const openExceptions=exceptions.filter(item=>['open','acknowledged'].includes(item.status));
  ...
  let currentExecutionLink:string|null=null;
  if(executionActive&&sessionUsable){
    try{currentExecutionLink=`${baseUrl}/execution/${createExecutionSessionToken(session.id)}`;}catch{currentExecutionLink=null;}
  }
      ...
      <div className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]" style={{paddingTop:18}}>
        <article className="card stack" style={{gap:20}}>
          ...
          <form action={saveExecutionAssignmentAction} className="stack" style={{gap:16}}>
            ...
            <button className="button" type="submit" disabled={noCommercialPartner||partnerMismatch||documents.length===0||!executionActive}>{assignment?'Save Partner setup':'Set up Partner work'}</button>
          </form>
        </article>

        <aside className="card stack" style={{gap:18}}><div><p className="eyebrow">Partner access</p><h2>Workspace access</h2><p>{executionActive?'Create or replace the Partner access link only when needed.':'Partner access is inactive while Overflow Partner owns review or closeout.'}</p></div>{assignment?<><dl className="project-os-summary" style={{margin:0}}><div><dt>Partner</dt><dd>{partnerName}</dd></div><div><dt>Recipient</dt><dd>{assignment.partner_contact_email}</dd></div><div><dt>Link status</dt><dd>{executionActive?(sessionUsable?'Active':session?stateLabel(session.status):'Not created'):'Inactive for current stage'}</dd></div><div><dt>Last opened</dt><dd>{session?.last_opened_at?formatDate(session.last_opened_at):'Never'}</dd></div></dl>{!executionActive?<div className="vp-callout"><strong>Partner work is not active.</strong><p>Overflow Partner currently owns the Project. Partner access will become available again only if a governed correction reopens Partner work.</p></div>:currentExecutionLink?<><a className="button" href={currentExecutionLink} target="_blank" rel="noreferrer">Open Partner workspace</a><label>Current Partner access link<input readOnly value={currentExecutionLink}/><small>Copy this URL when it needs to be sent to the assigned Partner.</small></label><details className="vp-disclosure"><summary>Replace Partner access link</summary><form action={generateExecutionLinkAction} style={{paddingTop:12}}><input type="hidden" name="project_id" value={id}/><button className="button secondary" type="submit" disabled={partnerMismatch||!assignment.scope_document_id}>Replace access link</button><small style={{display:'block',marginTop:8}}>Replacing the link revokes the current one. Do this only when the existing link should no longer be used.</small></form></details></>:sessionUsable?<div className="vp-callout"><strong>Partner link unavailable.</strong><p>The active Partner session could not be prepared for display. The rest of this Project view remains available; replace the link only if Partner access is actually required.</p></div>:<form action={generateExecutionLinkAction}><input type="hidden" name="project_id" value={id}/><button className="button" type="submit" disabled={partnerMismatch||!assignment.scope_document_id}>Create Partner access link</button></form>}<small>The access link only exposes this Partner's project context and lets them confirm start, report progress or exceptions, and submit delivery evidence.</small></>:<p>Complete Partner setup first.</p>}</aside>
      </div>
    </details>
  </section>;
}