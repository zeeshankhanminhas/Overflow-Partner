...
export default async function PartnerExecutionControlPage({ params, searchParams }: {
  ...
  const executionActive=Boolean(assignment&&commencement&&!partnerMismatch&&!noCommercialPartner);
  const currentHeadline=partnerMismatch?'Partner assignment needs attention':noCommercialPartner?'Commercial Partner is missing':!assignment?'Set up Partner execution':commencementPending?`Waiting for ${partnerName} to start`:`${partnerName} is executing Cycle ${String(cycle).padStart(2,'0')}`;
  const currentSummary=partnerMismatch?'The project assignment does not match the Partner selected through the accepted commercial position.':noCommercialPartner?'Execution cannot be set up until the accepted commercial position identifies the Partner.':!assignment?'Confirm the Partner, approved scope, dates and reporting cadence before giving the Partner workspace access.':commencementPending?'Partner access is ready. Execution starts only when the Partner confirms commencement in their workspace.':latestProgress?`Latest Partner update: ${stateLabel(latestProgress.progress_state)}${latestProgress.percent_complete===null||latestProgress.percent_complete===undefined?'':` · ${latestProgress.percent_complete}% reported`}.`:'Execution has started. No progress update has been received yet.';
  const nextAction=partnerMismatch||noCommercialPartner?'Resolve Partner assignment':!assignment?'Complete Partner setup':commencementPending?'Await Partner start confirmation':openExceptions.length?'Resolve Partner exception':submissions.length?'Review Partner delivery':'Await Partner update or delivery';
  const nextSummary=partnerMismatch||noCommercialPartner?'Return to the commercial/project record before releasing more work.':!assignment?'Use the setup section below to create the Partner assignment.':commencementPending?'Opening the Partner link is not commencement evidence; the Partner must confirm start.':openExceptions.length?`${openExceptions.length} Partner exception${openExceptions.length===1?'':'s'} need attention.`:submissions.length?`${submissions.length} delivery submission${submissions.length===1?' is':'s are'} available for delivery review.`:'No operator action is required unless the Partner reports a change, exception or delivery.';

  return <section className="stack" style={{gap:24,maxWidth:1180}}>
    <div><Link href={`/workspace/projects/${id}`} className="project-os-back">← Project 360</Link><p className="eyebrow" style={{marginTop:18}}>Partner execution</p><h1 style={{marginTop:8}}>{project.title}</h1><p className="lede">Track Partner start, progress, blockers and delivery. Use setup controls only when access, scope or dates need to change.</p></div>
    ...
    {query.execution_link?<div className="card" style={{borderLeft:'3px solid var(--op-success)'}}><strong>Partner access link created.</strong><p>It is available under Execution setup &amp; Partner access below.</p></div>:null}
    ...
    {noCommercialPartner?<div className="card" style={{borderLeft:'3px solid var(--op-danger)'}}><strong>Commercial Partner is missing.</strong><p>Project execution cannot be set up until the accepted commercial position identifies the Partner.</p></div>:null}

    <section className="operating-state" aria-label="Current Partner execution state">
      <div className="operating-state__copy">
        <div className="operating-state__kicker"><span className="product-provenance product-provenance--partner"><i/>Partner execution</span>{assignment?<span>Cycle {String(cycle).padStart(2,'0')}</span>:null}</div>
        <h2>{currentHeadline}</h2>
        <p>{currentSummary}</p>
        {executionActive?<div className="operating-state__actions">{currentExecutionLink?<a className="button secondary" href={currentExecutionLink} target="_blank" rel="noreferrer">Open Partner workspace →</a>:null}<Link className="button secondary" href={`/workspace/projects/${id}/delivery`}>Open delivery work →</Link></div>:null}
      </div>
    ...
    {assignment?<>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <article className="card stack"><div><p className="eyebrow">Partner delivery</p><h2>{submissions.length?`${submissions.length} received`:'Nothing received yet'}</h2><p>{submissions.length?'Delivery submissions are available for review in delivery work.':'The Partner has not submitted a delivery package yet.'}</p></div>{submissions.length?submissions.slice(0,5).map(item=><div key={item.id} style={{borderTop:'1px solid var(--op-border)',paddingTop:12}}><strong>{item.revision||`Cycle ${item.execution_cycle||1}`}</strong><p>{item.delivery_summary}</p><small>{formatDate(item.submitted_at)} · {stateLabel(item.review_status)}</small></div>):null}<Link className="button secondary" href={`/workspace/projects/${id}/delivery`}>Open delivery work →</Link></article>
      </div>
    </>:null}

    <details id="execution-setup" className="vp-disclosure" open={!assignment||partnerMismatch||noCommercialPartner}>
      <summary>Execution setup &amp; Partner access</summary>
      <div className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]" style={{paddingTop:18}}>
        <article className="card stack" style={{gap:20}}>
          <div><p className="eyebrow">Partner setup</p><h2>{assignment?'Partner assignment':'Set up Partner execution'}</h2><p>The Partner comes from the accepted commercial position. Confirm the approved scope, dates and reporting cadence used for this project.</p></div>
          <form action={saveExecutionAssignmentAction} className="stack" style={{gap:16}}>
            ...
            <button className="button" type="submit" disabled={noCommercialPartner||partnerMismatch||documents.length===0}>{assignment?'Save Partner setup':'Set up Partner execution'}</button>
          </form>
        </article>
