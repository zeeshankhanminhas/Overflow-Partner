...
export default async function PartnerExecutionControlPage({ params, searchParams }: {
  ...
  const partnerName=commercialPartner?.company_name||'Partner';
  const cycle=Number(assignment?.execution_cycle||1);
  const workActive=Boolean(assignment&&commencement&&!partnerMismatch&&!noCommercialPartner);
  const currentHeadline=partnerMismatch?'Partner assignment needs attention':noCommercialPartner?'Accepted Partner is missing':!assignment?'Set up Partner work':commencementPending?`Waiting for ${partnerName} to start`:`${partnerName} is working on Cycle ${String(cycle).padStart(2,'0')}`;
  const currentSummary=partnerMismatch?'The Partner on this project does not match the Partner selected through the accepted commercial position.':noCommercialPartner?'Partner work cannot be set up until the accepted commercial position identifies the Partner.':!assignment?'Confirm the Partner, approved scope, dates and reporting before giving the Partner access.':commencementPending?'Partner access is ready. Work starts only when the Partner confirms start in their workspace.':latestProgress?`Latest Partner update: ${stateLabel(latestProgress.progress_state)}${latestProgress.percent_complete===null||latestProgress.percent_complete===undefined?'':` · ${latestProgress.percent_complete}% reported`}.`:'Work has started. No progress update has been received yet.';
  const nextAction=partnerMismatch||noCommercialPartner?'Resolve Partner assignment':!assignment?'Complete Partner setup':commencementPending?'Await Partner start':openExceptions.length?'Resolve Partner issue':submissions.length?'Review Partner delivery':'Await Partner update or delivery';
  const nextSummary=partnerMismatch||noCommercialPartner?'Return to the commercial/project record before sending more work.':!assignment?'Use the setup section below to create the Partner assignment.':commencementPending?'Opening the Partner link does not count as starting; the Partner must confirm start.':openExceptions.length?`${openExceptions.length} Partner issue${openExceptions.length===1?' needs':'s need'} attention.`:submissions.length?`${submissions.length} delivery submission${submissions.length===1?' is':'s are'} available for review.`:'No operator action is required unless the Partner reports a change, issue or delivery.';

  return <section className="stack" style={{gap:24,maxWidth:1180}}>
    <div><Link href={`/workspace/projects/${id}`} className="project-os-back">← Project 360</Link><p className="eyebrow" style={{marginTop:18}}>Partner work</p><h1 style={{marginTop:8}}>{project.title}</h1><p className="lede">Track Partner start, progress, issues and delivery. Open setup only when access, scope, dates or reporting need to change.</p></div>
    ...
    {partnerMismatch?<div className="card" style={{borderLeft:'3px solid var(--op-danger)'}}><strong>Partner assignment does not match the accepted Partner.</strong><p>Resolve the Partner selection before sending more work.</p></div>:null}
    {noCommercialPartner?<div className="card" style={{borderLeft:'3px solid var(--op-danger)'}}><strong>Accepted Partner is missing.</strong><p>Resolve the commercial review before setting up Partner work.</p></div>:null}

    <section className="operating-state" aria-label="Current Partner work state">
      <div className="operating-state__copy">
        <div className="operating-state__kicker"><span className="product-provenance product-provenance--partner"><i/>Partner work</span>{assignment?<span>Cycle {String(cycle).padStart(2,'0')}</span>:null}</div>
        <h2>{currentHeadline}</h2>
        <p>{currentSummary}</p>
        {workActive?<div className="operating-state__actions">{currentExecutionLink?<a className="button secondary" href={currentExecutionLink} target="_blank" rel="noreferrer">Open Partner workspace →</a>:null}<Link className="button secondary" href={`/workspace/projects/${id}/delivery`}>Open delivery work →</Link></div>:null}
      </div>
      <div className="operating-state__decision">
        ...
        {(partnerMismatch||noCommercialPartner)?<div className="operating-state__actions"><Link className="button secondary" href={`/workspace/commercial-control?project=${id}`}>Open commercial →</Link></div>:null}
        {openExceptions.length?<div className="operating-state__actions"><a className="button secondary" href="#partner-exceptions">Open issues →</a></div>:null}
      </div>
    </section>

    {assignment?<div className="vp-facts"><div className="vp-fact"><small>Partner</small><strong>{partnerName}</strong></div><div className="vp-fact"><small>Start</small><strong>{commencement?formatDate(commencement.submitted_at):'Awaiting confirmation'}</strong></div><div className="vp-fact"><small>Latest update</small><strong>{latestProgress?formatDate(latestProgress.submitted_at):'Not received yet'}</strong></div><div className="vp-fact"><small>Partner forecast</small><strong>{formatDate(latestProgress?.forecast_delivery_date||commencement?.forecast_delivery_date||assignment.committed_due_date||project.due_date)}</strong></div></div>:null}

    {assignment?<>
      <article className="card stack" style={{gap:16}}><div><p className="eyebrow">Current progress</p><h2>{latestProgress?stateLabel(latestProgress.progress_state):commencement?'Work started':'Waiting to start'}</h2><p>{latestProgress?.work_in_progress||'No Partner progress update has been submitted yet.'}</p></div><div className="grid gap-4 md:grid-cols-4"><div><small>Start</small><strong style={{display:'block',marginTop:6}}>{commencement?formatDate(commencement.submitted_at):'Awaiting'}</strong></div><div><small>Partner state</small><strong style={{display:'block',marginTop:6}}>{latestProgress?stateLabel(latestProgress.progress_state):'No update'}</strong></div><div><small>Progress reported</small><strong style={{display:'block',marginTop:6}}>{latestProgress?.percent_complete===null||latestProgress?.percent_complete===undefined?'Not stated':`${latestProgress.percent_complete}%`}</strong></div><div><small>Forecast</small><strong style={{display:'block',marginTop:6}}>{formatDate(latestProgress?.forecast_delivery_date||commencement?.forecast_delivery_date)}</strong></div></div>{latestProgress?<small>Latest update received {formatDate(latestProgress.submitted_at)}.</small>:null}</article>

      <div className="grid gap-5 md:grid-cols-2">
        <article id="partner-exceptions" className="card stack"><div><p className="eyebrow">Partner issues</p><h2>{openExceptions.length?`${openExceptions.length} open`:'None open'}</h2><p>{openExceptions.length?'Resolve issues that are blocking or changing delivery.':'No Partner issues need attention.'}</p></div>{exceptions.length?exceptions.slice(0,5).map(item=><div key={item.id} style={{borderTop:'1px solid var(--op-border)',paddingTop:12}}><strong>{item.title}</strong><p>{stateLabel(item.severity)} · {stateLabel(item.status)} · {formatDate(item.raised_at)}</p>{['open','acknowledged'].includes(item.status)?<form action={resolveExecutionExceptionAction} className="stack" style={{gap:8,marginTop:10}}><input type="hidden" name="project_id" value={id}/><input type="hidden" name="exception_id" value={item.id}/><label>Resolution note<textarea name="resolution_note" rows={2} required placeholder="What resolved the issue?"/></label><button className="button secondary" type="submit">Resolve issue</button></form>:item.resolution_note?<small>Resolution · {item.resolution_note}</small>:null}</div>):null}</article>
        <article className="card stack"><div><p className="eyebrow">Partner delivery</p><h2>{submissions.length?`${submissions.length} received`:'Nothing received yet'}</h2><p>{submissions.length?'Partner deliveries are available for review in delivery work.':'The Partner has not submitted a delivery yet.'}</p></div>{submissions.length?submissions.slice(0,5).map(item=><div key={item.id} style={{borderTop:'1px solid var(--op-border)',paddingTop:12}}><strong>{item.revision||`Cycle ${item.execution_cycle||1}`}</strong><p>{item.delivery_summary}</p><small>{formatDate(item.submitted_at)} · {stateLabel(item.review_status)}</small></div>):null}<Link className="button secondary" href={`/workspace/projects/${id}/delivery`}>Open delivery work →</Link></article>
      </div>
    </>:null}
    ...
    <details id="execution-setup" className="vp-disclosure" open={!assignment||partnerMismatch||noCommercialPartner}>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]" style={{paddingTop:18}}>
        <article className="card stack" style={{gap:20}}>
          <div><p className="eyebrow">Partner setup</p><h2>{assignment?'Partner assignment':'Set up Partner work'}</h2><p>The Partner comes from the accepted commercial review. Confirm the approved scope, dates and reporting used for this project.</p></div>
          <form action={saveExecutionAssignmentAction} className="stack" style={{gap:16}}>
            <input type="hidden" name="project_id" value={id}/>
            <label>Partner<select name="partner_id" required defaultValue={commercialPartnerId||''}><option value={commercialPartnerId||''}>{commercialPartner?.company_name||'Accepted Partner unavailable'}</option></select><small>Inherited from the accepted commercial review. The Partner cannot be replaced here.</small></label>
