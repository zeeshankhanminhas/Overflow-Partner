...
import { humaniseOperatingReason } from '@/lib/presentation/operatingState';
import { resolveProjectJourneyPresentation } from '@/lib/presentation/projectOperatingState';
import { deliveryHealthLabel, projectPhaseForStage, projectPhaseMeta, projectPhaseOrder } from '@/lib/presentation/projectJourney';
...
import { ClientDeliveryRecordAction, ClientReviewOutcomeAction } from './ProjectClientDecisionActions';
...
export default async function ProjectWorkspacePage({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<Record<string,string|undefined>>}){
  ...
  const projectRecord=project as typeof project&{project_stage?:string};const stage=normaliseProjectStage(projectRecord.project_stage);const stageMeta=projectStageMeta[stage];const currentPhase=projectPhaseForStage(stage);const phaseIndex=projectPhaseOrder.indexOf(currentPhase);
  ...
  const financialGate=resolveFinancialGate(financialGateResult.data);const mobilisationControlsComplete=Boolean(project.project_manager_id&&project.start_date&&project.due_date);const partnerControlled=Boolean(executionAssignment);const partnerCommencementPending=partnerControlled&&stage==='ready_for_execution'&&!commencement;
  const financeHref=`/workspace/commercial-control?project=${id}&focus=financial-gate`;const deliveryHref=`/workspace/projects/${id}/delivery`;const executionHref=`/workspace/projects/${id}/execution`;const documentsHref=`/workspace/documents?project=${id}`;const paymentsHref=`/workspace/payments?project=${id}`;
  ...
  const partnerBlocker=partnerCommencementPending?partnerReason:null;const financialBlocker=stage==='mobilisation'&&!financialGate.authorised?`Financial authorisation: ${financialGate.reason}`:null;
  const blockers=[...nonDocumentReasons,...evidenceBlockers,...(partnerBlocker?[partnerBlocker]:[]),...(financialBlocker?[financialBlocker]:[])];

  const presentation=resolveProjectJourneyPresentation({
    stage,
    ...
    executionHref,deliveryHref,documentsHref,financeHref,
  });
  ...
  const header=<div style={{display:'flex',justifyContent:'space-between',gap:20,width:'100%',alignItems:'flex-start',flexWrap:'wrap'}}><div><Link href="/workspace/projects" className="project-os-back">← Projects</Link><p className="project-os-reference">Project 360 · {project.project_number}</p><h1>{project.title}</h1><div className="project-os-meta"><span>{value(lead,'company_name')}</span><span>Project manager · {value(manager,'full_name')}</span><span>Due · {formatDate(project.due_date)}</span>{presentation.waitingOn?<span>Waiting on · {presentation.waitingOn.label}</span>:null}</div></div></div>;
  const stateStrip=<div className="project-os-progress-card" style={{margin:0,border:0,padding:0,background:'transparent'}}><div className="project-os-progress-scroll"><ol className="project-os-progress">{projectPhaseOrder.map((item,index)=>{const state=index===phaseIndex?'current':'future';return <li key={item} className={state}><div className="project-os-node"><span>{index+1}</span></div><strong>{projectPhaseMeta[item].label}</strong>{state==='current'?<small>Current</small>:null}</li>})}</ol></div><div className="project-os-progress-footer"><p>{presentation.headline}</p><span>{presentation.waitingOn?`Current responsibility · ${presentation.waitingOn.label}`:partnerControlled?'Partner work complete':`${readiness.activityOpen} open activit${readiness.activityOpen===1?'y':'ies'}`}</span></div></div>;
  ...
  const readinessPanel=<div className="stack" style={{gap:10}}>
    {stage==='mobilisation'?<><div className="project-os-check-row"><span className={`project-os-check-icon ${project.project_manager_id?'':'missing'}`}>{project.project_manager_id?'✓':'!'}</span><div><strong>Project manager</strong><small>{project.project_manager_id?value(manager,'full_name'):'Assign a project manager'}</small></div><span>{project.project_manager_id?'Complete':'Required'}</span></div><div className="project-os-check-row"><span className={`project-os-check-icon ${project.start_date&&project.due_date?'':'missing'}`}>{project.start_date&&project.due_date?'✓':'!'}</span><div><strong>Programme dates</strong><small>{formatDate(project.start_date)} → {formatDate(project.due_date)}</small></div><span>{project.start_date&&project.due_date?'Complete':'Required'}</span></div></>:null}
    {resolvedEvidence.length>0?<div className="project-os-check-row"><span className={`project-os-check-icon ${evidenceBlockers.length?'missing':''}`}>{evidenceBlockers.length?'!':'✓'}</span><div><strong>Required documents</strong><small>{resolvedEvidence.filter(item=>item.satisfied).length} of {resolvedEvidence.length} complete</small></div><Link href={documentsHref}>{evidenceBlockers.length?'Resolve':'Open'}</Link></div>:null}
    {stage==='mobilisation'?<div className="project-os-check-row"><span className={`project-os-check-icon ${financialGate.authorised?'':'missing'}`}>{financialGate.authorised?'✓':'!'}</span><div><strong>Payment & commercial approval</strong><small>{financialGate.reason}</small></div><Link href={financeHref}>{financialGate.authorised?'Review':'Resolve'}</Link></div>:null}
    {partnerControlled&&['ready_for_execution','in_progress'].includes(stage)?<div className="project-os-check-row"><span className={`project-os-check-icon ${commencement?'':'missing'}`}>{commencement?'✓':'!'}</span><div><strong>Partner start confirmation</strong><small>{value(executionPartner,'company_name','Execution Partner')} · {commencement?`Confirmed ${formatDate(commencement.submitted_at)}`:'Awaiting Partner confirmation'}</small></div><Link href={executionHref}>{commencement?'Open':'Open workspace'}</Link></div>:null}
    {partnerControlled&&['in_progress','partner_correction'].includes(stage)?<div className="project-os-check-row"><span className={`project-os-check-icon ${currentCycleDelivery?'':'missing'}`}>{currentCycleDelivery?'✓':'·'}</span><div><strong>{stage==='partner_correction'?'Revised Partner delivery':'Partner delivery'}</strong><small>{currentCycleDelivery?`Received ${formatDate(currentCycleDelivery.submitted_at)}`:stage==='partner_correction'?'Awaiting revised delivery package':'Awaiting delivery package'}</small></div><Link href={executionHref}>Open</Link></div>:null}
    ...
    {(readiness.deliveryItems||0)>0?<div className="project-os-check-row"><span className={`project-os-check-icon ${(readiness.deliveryWorkOpen||0)>0?'missing':''}`}>{(readiness.deliveryWorkOpen||0)>0?'!':'✓'}</span><div><strong>Delivery work</strong><small>{readiness.deliveryItems} item{readiness.deliveryItems===1?'':'s'} · {readiness.deliveryWorkOpen||0} active/blocked</small></div><Link href={deliveryHref}>Open</Link></div>:null}
  ...
  const standardTransition=stageMeta.next?<form action={advanceProjectStageAction}><input type="hidden" name="project_id" value={project.id}/><input type="hidden" name="target_stage" value={stageMeta.next}/><label>Optional note<textarea name="note" rows={3} placeholder="Add context if needed"/></label><button className="button">{transitionLabel}</button>{stage==='internal_review'?<button className="button secondary" name="target_stage" value="partner_correction">Request Partner correction</button>:null}</form>:<div><strong>Project closed</strong><p>Delivery is complete.</p></div>;
  ...
  const clientTransmittalAction=<ClientDeliveryRecordAction projectId={id} recipientName={value(lead,'contact_name','')} recipientEmail={value(lead,'contact_email','')} blocked={preTransmittalBlockers.length>0} blocker={preTransmittalBlockers[0]?humaniseOperatingReason(preTransmittalBlockers[0]):undefined}/>;
  const clientReviewAction=<ClientReviewOutcomeAction projectId={id}/>;
  let nextAction:React.ReactNode;
  if(stage==='ready_for_client_issue')nextAction=clientTransmittalAction;
  else if(stage==='client_review'&&!readiness.clientOutcome)nextAction=clientReviewAction;
  else {const blockerHref=partnerCommencementPending?executionHref:(readiness.openPartnerExceptions||0)>0?executionHref:(readiness.deliveryWorkOpen||0)>0?deliveryHref:financialBlocker?financeHref:evidenceBlockers.length?documentsHref:'#record-readiness';nextAction=<GovernedAction state={actionState} blockedAction={<Link className="button secondary" href={blockerHref}>Resolve blocker{blockers.length===1?'':'s'} →</Link>}>{standardTransition}</GovernedAction>;}

  const summary=<dl className="project-os-summary" style={{margin:0}}><div><dt>Client</dt><dd>{value(lead,'company_name')}</dd></div><div><dt>Project manager</dt><dd>{value(manager,'full_name')}</dd></div><div><dt>Due</dt><dd>{formatDate(project.due_date)}</dd></div><div><dt>Value</dt><dd>{quote?money(quote.total,quote.currency):'Not recorded'}</dd></div></dl>;
  const activities=partnerControlled?<div className="stack operating-work-summary" style={{gap:12}}><div><strong>{presentation.headline}</strong><p>{presentation.summary}</p></div><div className="vp-facts"><div className="vp-fact"><small>Execution Partner</small><strong>{value(executionPartner,'company_name','Not recorded')}</strong></div><div className="vp-fact"><small>Current responsibility</small><strong>{presentation.waitingOn?.label||'Overflow Partner'}</strong></div><div className="vp-fact"><small>Delivery health</small><strong>{(readiness.openPartnerExceptions||0)>0?'Blocked':deliveryHealthLabel(latestPartnerProgress?.progress_state)}</strong></div><div className="vp-fact"><small>Partner update</small><strong>{latestPartnerProgress?formatDate(latestPartnerProgress.submitted_at):'Not received yet'}</strong></div></div>{latestPartnerProgress?<div className="project-os-check-row"><div><strong>{deliveryHealthLabel(latestPartnerProgress.progress_state)}</strong><small>{value(latestPartnerProgress,'work_in_progress','No current-work note')}</small></div><span>{latestPartnerProgress.percent_complete===null||latestPartnerProgress.percent_complete===undefined?'Partner reported':`${latestPartnerProgress.percent_complete}%`}</span></div>:null}<div style={{display:'flex',gap:10,flexWrap:'wrap'}}><Link className="button secondary" href={executionHref}>Open Partner workspace →</Link><Link className="button secondary" href={deliveryHref}>Open delivery work →</Link></div></div>:<div className="stack" style={{gap:12}}><div className="vp-facts"><div className="vp-fact"><small>Open activities</small><strong>{readiness.activityOpen}</strong></div><div className="vp-fact"><small>Total activities</small><strong>{tasks.length}</strong></div></div>{tasks.filter(t=>!['completed','cancelled'].includes(t.status)).slice(0,3).map(task=><div className="project-os-check-row" key={task.id}><div><strong>{task.title}</strong><small>{workspaceLabel(task.status,'task')} · {task.due_at?formatDate(task.due_at):'No due date'}</small></div></div>)}<Link className="button secondary" href={deliveryHref}>Open delivery work →</Link></div>;
  const evidence=resolvedEvidence.length?<div className="stack" style={{gap:12}}><div className="vp-facts"><div className="vp-fact"><small>Required now</small><strong>{resolvedEvidence.length}</strong></div><div className="vp-fact"><small>Complete</small><strong>{resolvedEvidence.filter(item=>item.satisfied).length}</strong></div><div className="vp-fact"><small>Blocking</small><strong>{resolvedEvidence.filter(item=>item.blocking).length}</strong></div></div>{resolvedEvidence.slice(0,4).map(item=><div className="project-os-check-row" key={item.key}><span className={`project-os-check-icon ${item.satisfied?'':'missing'}`}>{item.satisfied?'✓':'!'}</span><div><strong>{item.label}</strong><small>{item.operatorState}</small></div><Link href={`/workspace/documents/templates/${item.key}?project=${id}`}>{item.satisfied?'Open':'Resolve'}</Link></div>)}<Link className="button secondary" href={documentsHref}>Open Documents →</Link></div>:undefined;
  const communications=<div><p style={{color:'var(--op-muted)'}}>Project messages stay available here without crowding the current decision.</p><Link className="button secondary" href={`/workspace/communications/project/${id}`}>Open Messages →</Link></div>;
  ...
  const metadata=<dl className="project-os-summary" style={{margin:0}}><div><dt>Project ID</dt><dd>{project.id}</dd></div><div><dt>Created</dt><dd>{formatDate(project.created_at)}</dd></div><div><dt>Updated</dt><dd>{formatDate(project.updated_at)}</dd></div>{project.lead_id?<div><dt>Source Case</dt><dd><Link href={`/workspace/leads/${project.lead_id}`}>Open Case 360</Link></dd></div>:null}{readiness.clientOutcome?<div><dt>Client outcome</dt><dd>{stateLabel(readiness.clientOutcome)}</dd></div>:null}</dl>;
