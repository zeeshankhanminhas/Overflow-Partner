...
import { humaniseOperatingReason, resolveProjectPresentation } from '@/lib/presentation/operatingState';
...
export default async function ProjectWorkspacePage({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<Record<string,string|undefined>>}){
  ...
  const readinessPanel=<div className="stack" style={{gap:10}}>
    ...
    {partnerControlled&&['in_progress','partner_correction'].includes(stage)?<div className="project-os-check-row"><span className={`project-os-check-icon ${currentCycleDelivery?'':'missing'}`}>{currentCycleDelivery?'✓':'·'}</span><div><strong>Partner delivery · Cycle {Number(executionAssignment?.execution_cycle||readiness.executionCycle||1)}</strong><small>{currentCycleDelivery?`Received ${formatDate(currentCycleDelivery.submitted_at)}`:'Awaiting current-cycle delivery package'}</small></div><Link href={executionHref}>Open</Link></div>:null}
