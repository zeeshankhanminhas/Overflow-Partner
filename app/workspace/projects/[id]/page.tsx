...
export default async function ProjectWorkspacePage({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<Record<string,string|undefined>>}){
  ...
  const blockers=[...nonDocumentReasons,...evidenceBlockers,...(financialBlocker?[financialBlocker]:[])];
  const actionState=resolveActionState({label:stageMeta.action,businessReady:readiness.ready&&resolvedEvidence.every(item=>item.satisfied)&&!financialBlocker,blockers,readyMessage:'All requirements for this transition are satisfied.'});
  ...
  const blockerHref=financialBlocker?financeHref:evidenceBlockers.length?documentsHref:'#record-readiness';
  const nextAction=<GovernedAction state={actionState} blockedAction={<Link className="button secondary" href={blockerHref}>Resolve blocker{blockers.length===1?'':'s'} →</Link>}>{permittedAction}</GovernedAction>;
  ...
  const audit=<div className="stack"><Link href={documentsHref}>Document register →</Link><Link href={paymentsHref}>Payments →</Link><Link href={financeHref}>Commercial control →</Link></div>;
