...
export default async function ProjectWorkspacePage({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<Record<string,string|undefined>>}){
  ...
  const basePresentation=resolveProjectPresentation({
    ...
    executionHref,deliveryHref,documentsHref,financeHref:financialGate.authorised?financeHref:paymentsHref,
  });
  const presentation:OperatingPresentation=stage==='mobilisation'&&!financialGate.authorised?{
    ...
    nextAction:{label:'Record payment',available:true,kind:'navigate',href:paymentsHref,reason:'Record cleared settlement in Payments. Project execution remains locked until the required amount has been received.'},
    ...
    primaryActions:[{label:'Open Payments',href:paymentsHref,available:true,kind:'navigate'}],
  }:basePresentation;
  ...
  const stateStrip=<div className="project-os-progress-card" style={{margin:0,border:0,padding:0,background:'transparent'}}><div className="project-os-progress-scroll"><ol className="project-os-progress">{projectStages.map((item,index)=>{const state=index<stageIndex?'complete':index===stageIndex?'current':'future';return <li key={item} className={state}><div className="project-os-node"><span>{index+1}</span></div><strong>{projectStageMeta[item].label}</strong>{state==='current'?<small>Current</small>:null}</li>})}</ol></div><div className="project-os-progress-footer"><p>{presentation.headline}</p><span>{partnerControlled?`Cycle ${Number(executionAssignment?.execution_cycle||readiness.executionCycle||1)} · Partner ${stateLabel(executionAssignment?.execution_state)}`:`${readiness.activityOpen} open activit${readiness.activityOpen===1?'y':'ies'}`}</span></div></div>;
  ...
  const readinessPanel=<div className="stack" style={{gap:10}}>
    ...
    {stage==='mobilisation'?financialGate.authorised?<div className="project-os-check-row"><span className="project-os-check-icon">✓</span><div><strong>Start payment received</strong><small>{money(financialGate.received,financialGate.currency)} cleared · requirement {money(financialGate.required,financialGate.currency)}</small></div><Link href={paymentsHref}>Open payments</Link></div>:<div className="project-os-check-row"><span className="project-os-check-icon missing">!</span><div><strong>Start payment required</strong><small>Required to start: {money(financialGate.required,financialGate.currency)} · Received: {money(financialGate.received,financialGate.currency)}</small></div><Link href={paymentsHref}>Record payment</Link></div>:null}
    {partnerControlled&&['ready_for_execution','in_progress'].includes(stage)?<div className="project-os-check-row"><span className={`project-os-check-icon ${commencement?'':'missing'}`}>{commencement?'✓':'!'}</span><div><strong>Partner commencement</strong><small>{value(executionPartner,'company_name','Execution Partner')} · {commencement?`Confirmed ${formatDate(commencement.submitted_at)}`:'Awaiting Partner confirmation'}</small></div><Link href={executionHref}>{commencement?'Open':'Open workspace'}</Link></div>:null}
    {partnerControlled&&['in_progress','partner_correction'].includes(stage)?<div className="project-os-check-row"><span className={`project-os-check-icon ${currentCycleDelivery?'':'missing'}`}>{currentCycleDelivery?'✓':'·'}</span><div><strong>Cycle {Number(executionAssignment?.execution_cycle||readiness.executionCycle||1)} Partner delivery</strong><small>{currentCycleDelivery?`Received ${formatDate(currentCycleDelivery.submitted_at)}`:'Awaiting current-cycle delivery package'}</small></div><Link href={executionHref}>Open</Link></div>:null}
  ...
  const standardTransition=stageMeta.next?<form action={advanceProjectStageAction}><input type="hidden" name="project_id" value={project.id}/><input type="hidden" name="target_stage" value={stageMeta.next}/><label>Decision note<textarea name="note" rows={3} placeholder="Optional transition note"/></label><button className="button">{transitionLabel}</button>{stage==='internal_review'?<button className="button secondary" name="target_stage" value="partner_correction">Request Partner correction</button>:null}</form>:<div><strong>Project closed</strong><p>Delivery is complete.</p></div>;
  ...
  const clientTransmittalAction=<div className="stack"><div className="vp-callout"><strong>Send approved delivery to client</strong><p>Record who received the controlled package, how it was sent and when. This evidence advances the Project to Sent to client.</p></div><form action={recordClientTransmittalAction} className="stack"><input type="hidden" name="project_id" value={id}/><div className="grid gap-4 md:grid-cols-2"><label>Recipient name<input name="recipient_name" required defaultValue={value(lead,'contact_name','')}/></label><label>Recipient email<input name="recipient_email" type="email" defaultValue={value(lead,'contact_email','')}/></label></div><label>Delivery method<select name="delivery_method" required defaultValue="email"><option value="email">Email</option><option value="secure_link">Secure link</option><option value="client_portal">Client portal</option><option value="other">Other controlled method</option></select></label><label>Transmittal note<textarea name="note" rows={3} placeholder="Optional issue note"/></label><button className="button" disabled={preTransmittalBlockers.length>0}>Record transmittal and send</button></form>{preTransmittalBlockers.length>0?<p>Resolve first: {humaniseOperatingReason(preTransmittalBlockers[0])}</p>:null}</div>;
  const clientReviewAction=<div className="stack"><div className="vp-callout"><strong>Record the client's actual outcome</strong><p>Use identifiable written evidence. Accepted outcomes permit Completion once other gates are clear; changes or rejection route the Project into a new Partner correction cycle.</p></div><form action={recordClientReviewOutcomeAction} className="stack"><input type="hidden" name="project_id" value={id}/><label>Client outcome<select name="outcome" required defaultValue=""><option value="" disabled>Select outcome</option><option value="accepted">Accepted</option><option value="accepted_with_comments">Accepted with comments</option><option value="changes_requested">Changes requested</option><option value="rejected">Rejected / correction required</option></select></label><label>Evidence basis<select name="evidence_basis" required defaultValue="email_confirmation"><option value="email_confirmation">Email confirmation</option><option value="signed_acceptance">Signed acceptance</option><option value="client_portal">Client portal</option><option value="meeting_record">Meeting record</option><option value="other_written">Other written evidence</option></select></label><label>Evidence reference<input name="evidence_reference" required placeholder="Email subject/date, signed acceptance ref, meeting record..."/></label><label>Comments<textarea name="comments" rows={3}/></label><button className="button">Record client outcome</button></form></div>;
  ...
  else {const blockerHref=partnerCommencementPending?executionHref:(readiness.openPartnerExceptions||0)>0?executionHref:(readiness.deliveryWorkOpen||0)>0?deliveryHref:financialBlocker?paymentsHref:evidenceBlockers.length?documentsHref:'#record-readiness';nextAction=<GovernedAction state={actionState} blockedAction={<Link className="button secondary" href={blockerHref}>{financialBlocker?'Record payment':'Resolve blocker'}{!financialBlocker&&blockers.length!==1?'s':''} →</Link>}>{standardTransition}</GovernedAction>;}
