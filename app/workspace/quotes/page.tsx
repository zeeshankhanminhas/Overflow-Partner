...
export default async function Page({ searchParams }: { searchParams?: Promise<Record<string,string|undefined>> }) {
  ...
  return <section className="vp-page">
    <ProductPageHeader eyebrow="Commercial · Quotes" title="Client quotes" description="Controlled quotations generated from an approved commercial position. The register shows whether each quote needs work, approval or a client response." backHref={params.lead?`/workspace/leads/${params.lead}`:undefined} backLabel="Back to Case" actions={<><Link className="button secondary" href="/workspace/approvals">Approvals</Link><Link className="button secondary" href="/workspace/payments">Payments</Link></>} />
    ...
    <section>
      ...
      {quotes.length===0?<ProductEmptyState title="No Client Quotes yet" description="Complete the technical basis and commercial decision from the relevant Case." action={<Link className="button secondary" href="/workspace/leads?view=commercial-review">Open Cases</Link>} />:<ProductRegister>
        {quoteRows.map(({quote,presentation})=><ProductRegisterRow key={quote.id} href={`/workspace/leads/${quote.lead_id}`} className={selected?.id===quote.id?'is-selected':''}>
          ...
          <strong>{presentation.nextAction.kind==='wait'?'Open context →':'Review →'}</strong>
        </ProductRegisterRow>)}
      </ProductRegister>}
    </section>
  </section>;
