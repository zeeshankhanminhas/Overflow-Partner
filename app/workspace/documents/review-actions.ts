...
export async function signControlledDocumentAction(
  documentId: string,
  signerName: string,
  signerRole: string,
  ...
  try {
    ...
    if (['signed', 'approved', 'issued', 'published'].includes(document.status)) {
      return { ok: true, message: 'Document is already signed.', status: document.status };
    }
    if (!['draft', 'in_review', 'changes_requested'].includes(document.status)) {
      return { ok: false, message: `Document cannot be signed from ${document.status}.` };
    }
    ...
    return { ok: false, message: error instanceof Error ? error.message : 'Document signing failed.' };
  }
}
...
export async function requestDocumentChangesAction(documentId: string, reason: string): Promise<DocumentReviewResult> {
  try {
    ...
    if (error || !document) return { ok: false, message: 'Controlled document could not be found.' };
    if (!['draft', 'in_review', 'signed'].includes(document.status)) {
      return { ok: false, message: `Changes cannot be requested from ${document.status}.` };
    }

    const requestedAt = new Date().toISOString();
    const { error: updateError } = await supabase
      .from('documents')
      .update({ status: 'changes_requested', updated_at: requestedAt })
      .eq('organisation_id', organisationId)
      .eq('id', documentId);
    ...
    await recordDocumentEvent({
      ...
      userId: user.id,
      document,
      eventType: 'document.changes_requested',
      oldStatus: document.status,
      newStatus: 'changes_requested',
      evidence: { reason: reason.trim(), requested_at: requestedAt },
    });

    revalidateDocumentOwners(document);
    return { ok: true, message: 'Changes requested and audit event recorded.', status: 'changes_requested' };
  } catch (error) {
    return { ok: false, message: error instanceof Error ? error.message : 'Change request failed.' };
  }
}
...
export async function approveControlledDocumentAction(documentId: string): Promise<DocumentReviewResult> {
  try {
    ...
    if (document.status !== 'signed') {
      return { ok: false, message: 'Document must be electronically signed before approval.' };
    }
    ...
    await recordDocumentEvent({
      ...
      oldStatus: document.status,
      newStatus: 'approved',
      evidence: { approved_at: approvedAt, approver_email: user.email, approver_role: profile.role },
    });
