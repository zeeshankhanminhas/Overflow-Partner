...
export default async function Case360Page({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<Record<string,string|string[]|undefined>>}){
  ...
  if(workflow.commercialReview&&!workflow.clientQuote){currentStatus='Quote ready to generate';nextAction='Generate client quote';reason='The approved commercial position can now become a controlled quote.';actionKey='generate_quote'}
  if(workflow.clientQuote&&['draft','internal_review'].includes(workflow.clientQuote.status)){currentStatus='Draft quote ready';nextAction='Issue client quote';reason='The quote is ready to be sent to the client.';actionKey='issue_quote'}
  if(workflow.clientQuote?.status==='issued'&&!workflow.project){currentStatus='Awaiting client decision';nextAction='Record client outcome';reason='The issued quote is awaiting acceptance or rejection.';actionKey='accept_quote'}
  ...
  const primaryTitle=actionKey==='accept_quote'?'Issued client quote':actionKey==='issue_quote'||actionKey==='generate_quote'?'Client quote':actionKey==='decide_partner'?'Partner response':actionKey==='create_commercial'?'Commercial position':actionKey==='create_partner_review'||actionKey==='wait_partner'?'Partner review':'Technical scope';
  ...
  return <section className="vp-page">
    ...
    <div className="vp-primary-grid">
      <main className="vp-object vp-object--hero">
        <span className="vp-status">{currentStatus}</span>
        <p className="vp-label">Primary object</p><h2 style={{fontSize:'clamp(1.8rem,3vw,3rem)',margin:'.5rem 0'}}>{primaryTitle}</h2><p style={{maxWidth:680,color:'rgb(255 255 255/.58)',lineHeight:1.65}}>{reason}</p>

        {quote?<div className="vp-facts"><Fact label="Quote" value={quote.quote_number}/><Fact label="Subtotal" value={money(quote.currency,quote.subtotal)}/><Fact label="VAT" value={money(quote.currency,quote.vat)}/><Fact label="Total" value={money(quote.currency,quote.total)}/><Fact label="Valid until" value={quote.valid_until}/><Fact label="Status" value={workspaceLabel(quote.status,'clientQuote')}/></div>:null}
      ...
      <aside className="vp-action"><p className="vp-label">Next action</p><h2>{nextAction}</h2>{blocker?<p><strong>Blocked:</strong> {blocker}</p>:<p>{reason}</p>}
        ...
        {actionKey==='create_partner_review'?<form action={createPartnerReviewRequestAction} className="stack"><input type="hidden" name="lead_id" value={id}/><input type="hidden" name="technical_intake_id" value={workflow.technicalIntake?.id}/><label>Execution partner<select name="partner_id" required defaultValue=""><option value="">Select partner</option>{partners.map((p:any)=><option key={p.id} value={p.id}>{p.company_name}</option>)}</select></label><label>Response due<input name="response_due_at" type="datetime-local" required/></label><label>Files<select name="file_selection" defaultValue="all"><option value="all">All approved documents</option><option value="none">No files</option></select></label><details className="vp-disclosure"><summary>Advanced controls</summary><div className="stack"><label>Optional instructions<textarea name="review_instructions" rows={3}/></label><label>Client identity<select name="show_client_identity" defaultValue="false"><option value="false">Hide</option><option value="true">Show</option></select></label><label>Commercial identity<select name="show_commercial_identity" defaultValue="false"><option value="false">Hide</option><option value="true">Show</option></select></label></div></details><button className="button">Send partner review</button></form>:null}
        {actionKey==='decide_partner'?<form action={decidePartnerReviewAction} className="stack"><input type="hidden" name="lead_id" value={id}/><input type="hidden" name="request_id" value={review.id}/><input type="hidden" name="response_id" value={response.id}/><label>Decision<select name="decision" required defaultValue=""><option value="">Select decision</option><option value="approved">Approve</option><option value="approved_with_conditions">Approve with conditions</option><option value="clarification_required">Request clarification</option><option value="rejected">Reject</option></select></label><label>Decision note<textarea name="review_notes" rows={3}/></label><details className="vp-disclosure"><summary>Clarification</summary><div><textarea name="clarification_request" rows={3}/></div></details><button className="button">Record decision</button></form>:null}
        ...
        {actionKey==='accept_quote'?<form action={acceptQuoteAction}><input type="hidden" name="lead_id" value={id}/><input type="hidden" name="quote_id" value={quote?.id}/><button className="button">Record acceptance and create project</button></form>:null}
