...
export async function setCommercialTermsAction(formData:FormData){
  ...
  try{
    ...
    await audit(supabase,organisationId,user.id,'project',projectId,'commercial_terms_updated',{basis,requiredAmount,paymentTermsDays:payload.payment_terms_days});refresh(projectId);destination=url({project:projectId,updated:'Commercial terms saved.',focus:'financial-gate'});
...
export async function createInvoiceAction(formData:FormData){
  ...
  try{
    ...
    await audit(supabase,organisationId,user.id,'project',projectId,'invoice_created',{invoiceId:invoice.id,invoiceNumber:invoice.invoice_number,total});refresh(projectId);destination=url({project:projectId,created:`Invoice ${invoice.invoice_number} created.`,focus:`invoice-${invoice.id}`});
...
export async function issueInvoiceAction(formData:FormData){
  ...
  try{
    const {supabase,user,profile,organisationId}=await requireUserContext();assertRole(profile.role,[...financeRoles]);
    const {data:invoice,error:readError}=await supabase.from('invoices').select('id,project_id,lead_id,invoice_number,status,due_date,total,currency,public_token').eq('organisation_id',organisationId).eq('id',invoiceId).single();if(readError||!invoice)throw new Error(readError?.message||'Invoice not found.');if(invoice.status!=='draft')throw new Error('Only a draft invoice can be issued.');projectId=invoice.project_id;
...
export async function recordClientPaymentAction(formData:FormData){
  ...
  try{
    ...
    await audit(supabase,organisationId,user.id,'project',invoice.project_id,'client_payment_recorded',{invoiceId,invoiceNumber:invoice.invoice_number,amount,remainingBalance:remaining});refresh(invoice.project_id);destination=url({project:projectId,updated:`Payment recorded against ${invoice.invoice_number}.`,focus:`invoice-${invoice.id}`});
...
export async function createPartnerPayableAction(formData:FormData){
  ...
  try{
    ...
    await audit(supabase,organisationId,user.id,'project',projectId,'partner_payable_received',{payableId:payable.id,payableNumber:payable.payable_number,total});refresh(projectId);destination=url({project:projectId,created:`Partner payable ${payable.payable_number} recorded.`,focus:`payable-${payable.id}`});
...
export async function approvePartnerPayableAction(formData:FormData){
  ...
  try{
    ...
    await audit(supabase,organisationId,user.id,'project',payable.project_id,'partner_payable_approved',{payableId,payableNumber:payable.payable_number,total:payable.total});refresh(payable.project_id);destination=url({project:projectId,updated:`${payable.payable_number} approved for payment.`,focus:`payable-${payable.id}`});
...
export async function recordPartnerPaymentAction(formData:FormData){
  ...
  try{
    ...
    await audit(supabase,organisationId,user.id,'project',payable.project_id,'partner_payment_recorded',{payableId,payableNumber:payable.payable_number,amount});refresh(payable.project_id);destination=url({project:projectId,updated:`Payment recorded against ${payable.payable_number}.`,focus:`payable-${payable.id}`});
