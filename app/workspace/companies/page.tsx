...
export default async function CompaniesPage({searchParams}:{searchParams?:Promise<Record<string,string|string[]|undefined>>}){
  ...
  return <section className="vp-page">
    ...
    <section><ProductSectionHeader eyebrow="CRM register" title="Companies" actions={addCompany} />{companies.length===0?<ProductEmptyState title="No companies yet" description="Add the first client or prospect company to create the master record used by the rest of the workspace." action={addCompany}/>:<ProductRegister>{companies.map(company=><ProductRegisterRow key={company.id}>
