...
export default async function CompaniesPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <section>
      ...
      <h1>Companies</h1>
      <p className="lede">Maintain the master company record used by prospects, leads, quotes, projects and documents.</p>
      ...
      <div style={{ marginTop: 32, display: 'grid', gap: 12 }}>
        {companies.length === 0 ? (
          <div className="card" style={{ width: '100%' }}><h3>No companies yet</h3><p className="lede" style={{ fontSize: 16 }}>Add the first client or prospect company above.</p></div>
        ) : companies.map((company) => (
          <article className="metric" key={company.id}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'flex-start' }}>
              <div>
                <strong style={{ marginTop: 0, fontSize: 22 }}>{company.name}</strong>
                <p>{[company.industry, company.country].filter(Boolean).join(' · ') || 'Profile details not added'}</p>
              </div>
              {company.website ? <a href={company.website} target="_blank" rel="noreferrer">Website</a> : null}
            </div>
