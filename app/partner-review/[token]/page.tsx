...
export default function PartnerReviewPage() {
  ...
  useEffect(() => {
    fetch(`/api/partner-review/${token}`).then(async (response) => {
      ...
      setState(body);
      setLoading(false);
    }).catch(() => { setState({ message: 'Unable to load this Partner assessment.' }); setLoading(false); });
  }, [token]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage('Technical and commercial response submitted. Overflow Partner will review it and contact you if anything needs clarification.');
  ...
  return <main className="min-h-screen bg-[#f5f4ef] px-6 py-14 md:px-10 md:py-20">
    <div className="mx-auto grid max-w-7xl gap-14 border-t border-black/10 pt-10 lg:grid-cols-[0.9fr_1.45fr]">
      <aside className="grid content-start gap-8 lg:sticky lg:top-8 lg:self-start">
        <div><p className="text-xs font-medium uppercase tracking-[0.12em] text-neutral-500">Overflow Partner · Partner assessment</p><h1 className="mt-5 text-4xl font-semibold tracking-[-0.035em] md:text-6xl">Technical &amp; commercial assessment</h1><p className="mt-5 max-w-xl leading-7 text-neutral-600">Review the scope, confirm capability and capacity, then provide price and delivery terms.</p></div>
        <div className="grid gap-5 border-t border-black/10 pt-7">
          <div><p className="text-xs uppercase tracking-[0.08em] text-neutral-500">Reference</p><p className="mt-2 text-xl font-medium">{state.review.case_reference}</p></div>
          ...
          {state.review.client_identity ? <div><p className="text-xs uppercase tracking-[0.08em] text-neutral-500">Client</p><p className="mt-2">{state.review.client_identity}</p></div> : null}
          <div className="grid grid-cols-2 gap-6 border-t border-black/10 pt-6"><div><p className="text-xs uppercase tracking-[0.08em] text-neutral-500">Response due</p><p className="mt-2 text-sm">{new Date(state.review.response_due_at).toLocaleDateString('en-GB')}</p></div><div><p className="text-xs uppercase tracking-[0.08em] text-neutral-500">NDA</p><p className="mt-2 text-sm">{state.partner?.nda_signed ? 'Confirmed' : 'Not confirmed'}</p></div></div>
        </div>
        <div className="border-l-2 border-orange-500 pl-5 text-sm leading-6 text-neutral-600">Your price is visible only to Overflow Partner and is not shown directly to the client.</div>
      </aside>

      {locked ? <section className="grid content-start gap-6 border-t border-black/10 pt-10"><p className="text-xs uppercase tracking-[0.12em] text-neutral-500">Response received</p><h2 className="text-3xl font-semibold">Thank you. Your assessment has been submitted.</h2><p className="leading-7 text-neutral-600">Your technical and commercial response has been saved. Overflow Partner will contact you if anything needs clarification.</p></section> : <form className="grid gap-10" onSubmit={submit}>
        <section className={section}><div><p className="text-xs uppercase tracking-[0.12em] text-neutral-500">01 · Scope &amp; files</p><h2 className="mt-2 text-2xl font-semibold">Review the engineering context.</h2></div>
          ...
          {state.review.review_instructions ? <div><p className="text-xs uppercase tracking-[0.08em] text-neutral-500">Instructions</p><p className="mt-2 whitespace-pre-wrap leading-7">{state.review.review_instructions}</p></div> : null}
          <div><p className="text-xs uppercase tracking-[0.08em] text-neutral-500">Shared files</p>{state.files?.length ? <div className="mt-3 grid gap-2">{state.files.map((file) => <div key={file.id} className="border-b border-black/10 py-3 text-sm">{file.display_name}</div>)}</div> : <p className="mt-2 text-sm text-neutral-500">No files were attached to this assessment.</p>}</div>
        </section>

        {state.clarification ? <section className="border-l-2 border-orange-500 pl-5"><p className="text-xs uppercase tracking-[0.08em] text-neutral-500">Clarification · Revision {state.clarification.revision_number}</p><p className="mt-2 whitespace-pre-wrap leading-7">{state.clarification.clarification_request}</p></section> : null}

        <section className={section}><div><p className="text-xs uppercase tracking-[0.12em] text-neutral-500">02 · Capability</p><h2 className="mt-2 text-2xl font-semibold">Can you deliver this work?</h2></div>
        ...
        <section className={section}><div><p className="text-xs uppercase tracking-[0.12em] text-neutral-500">03 · Delivery</p><h2 className="mt-2 text-2xl font-semibold">Estimate effort, timing and conditions.</h2></div>
        ...
        <section className={section}><div><p className="text-xs uppercase tracking-[0.12em] text-neutral-500">04 · Price &amp; terms</p><h2 className="mt-2 text-2xl font-semibold">Provide your price and delivery terms.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-neutral-600">Price and currency are required when the scope is feasible. Leave them blank when more information is required or the work is not feasible.</p></div>
          <div className="grid gap-8 md:grid-cols-2"><label className={label}>Price<input className={field} name="commercial_price" type="number" min="0.01" step="0.01" /></label><Select name="commercial_currency" title="Currency" defaultValue="GBP" options={[["GBP","GBP"],["USD","USD"],["EUR","EUR"],["AED","AED"]]} /><label className={label}>Quote reference<input className={field} name="quote_reference" /></label><label className={label}>Valid until<input className={field} name="commercial_valid_until" type="date" /></label></div>
        ...
        <section className={section}><div><p className="text-xs uppercase tracking-[0.12em] text-neutral-500">05 · Confirmation</p><h2 className="mt-2 text-2xl font-semibold">Confirm who completed the assessment.</h2></div><div className="grid gap-8 md:grid-cols-2"><label className={label}>Reviewer name<input className={field} name="reviewer_name" required /></label><label className={label}>Reviewer role<input className={field} name="reviewer_role" /></label></div><label className="flex items-start gap-3 text-sm leading-6"><input className="mt-1" type="checkbox" name="declaration_checked" value="true" required />I confirm that I reviewed the scope shown above and that this response accurately states our capability, assumptions, risks, capacity, price and terms.</label><button className="min-h-12 bg-black px-7 py-3 text-sm font-medium uppercase tracking-[0.08em] text-white disabled:bg-neutral-500" disabled={submitting}>{submitting ? 'Submitting' : 'Submit assessment'}</button>{message ? <p className="border border-black/10 bg-white/50 p-4 text-sm" role="status">{message}</p> : null}</section>
      </form>}
    </div>
  </main>;
}