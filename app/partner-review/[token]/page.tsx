...
function Select({ name, title, options, required = false }: { name: string; title: string; options: Array<[string,string]>; required?: boolean }) {
  return <label className={label}>{title}<select className={`${field} normal-case tracking-normal text-black`} name={name} required={required} defaultValue=""><option value="">Select an option</option>{options.map(([value,text]) => <option key={value} value={value}>{text}</option>)}</select></label>;
}
...
export default function PartnerReviewPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) return setMessage(body.message || 'Unable to submit the technical response.');
    setMessage('Technical response submitted. Overflow Partner will now complete its internal review.');
    setState((current) => ({ ...current, review: current.review ? { ...current.review, status: 'submitted' } : undefined, latest_response: body.response }));
  }
  ...
  return <main className="min-h-screen bg-[#f5f4ef] px-6 py-14 md:px-10 md:py-20">
    <div className="mx-auto grid max-w-7xl gap-14 border-t border-black/10 pt-10 lg:grid-cols-[0.9fr_1.45fr]">
      <aside className="grid content-start gap-8 lg:sticky lg:top-8 lg:self-start">
        <div><p className="text-xs font-medium uppercase tracking-[0.12em] text-neutral-500">Overflow Partner · Controlled review</p><h1 className="mt-5 text-4xl font-semibold tracking-[-0.035em] md:text-6xl">Technical partner review</h1><p className="mt-5 max-w-xl leading-7 text-neutral-600">Assess feasibility, capacity, risk and delivery readiness before a commercial response is requested.</p></div>
        ...
        <div className="border-l-2 border-orange-500 pl-5 text-sm leading-6 text-neutral-600">This portal contains only the controlled engineering information approved for this review. Client pricing, margins and internal strategy are not disclosed.</div>
      </aside>

      {locked ? <section className="grid content-start gap-6 border-t border-black/10 pt-10"><p className="text-xs uppercase tracking-[0.12em] text-neutral-500">Response locked</p><h2 className="text-3xl font-semibold">Technical response received.</h2><p className="leading-7 text-neutral-600">Submitted responses are immutable. Overflow Partner will issue a formal clarification request if revision is required.</p></section> : <form className="grid gap-10" onSubmit={submit}>
