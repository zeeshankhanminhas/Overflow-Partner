...
-- Overflow Partner canonical lifecycle prerequisites.
-- docs/BUSINESS_LIFECYCLE_CONSTITUTION.md is the normative model.

-- ------------------------------------------------------------------
-- Client commercial/delivery evidence
-- ------------------------------------------------------------------
...
drop policy if exists quote_acceptance_records_org_access on public.quote_acceptance_records;
create policy quote_acceptance_records_org_access on public.quote_acceptance_records for all
using (exists(select 1 from public.profiles p where p.id=auth.uid() and p.organisation_id=quote_acceptance_records.organisation_id and p.is_active=true))
with check (exists(select 1 from public.profiles p where p.id=auth.uid() and p.organisation_id=quote_acceptance_records.organisation_id and p.is_active=true));

drop policy if exists project_client_transmittals_org_access on public.project_client_transmittals;
create policy project_client_transmittals_org_access on public.project_client_transmittals for all
using (exists(select 1 from public.profiles p where p.id=auth.uid() and p.organisation_id=project_client_transmittals.organisation_id and p.is_active=true))
with check (exists(select 1 from public.profiles p where p.id=auth.uid() and p.organisation_id=project_client_transmittals.organisation_id and p.is_active=true));

drop policy if exists project_client_reviews_org_access on public.project_client_reviews;
create policy project_client_reviews_org_access on public.project_client_reviews for all
using (exists(select 1 from public.profiles p where p.id=auth.uid() and p.organisation_id=project_client_reviews.organisation_id and p.is_active=true))
with check (exists(select 1 from public.profiles p where p.id=auth.uid() and p.organisation_id=project_client_reviews.organisation_id and p.is_active=true));

-- ------------------------------------------------------------------
-- Revision-aware external execution
-- ------------------------------------------------------------------
...
create index if not exists partner_delivery_submissions_cycle_idx on public.partner_delivery_submissions(assignment_id,execution_cycle,submitted_at desc);

-- ------------------------------------------------------------------
-- Canonical controlled-document names
-- ------------------------------------------------------------------
update public.documents legacy
set status='superseded'::public.document_status,is_current_revision=false,superseded_at=coalesce(legacy.superseded_at,now()),updated_at=now()
where legacy.document_type='scope_of_work' and exists(
  select 1 from public.documents canonical
  where canonical.organisation_id=legacy.organisation_id and canonical.document_type='scope-of-work' and canonical.status::text<>'superseded'
...
update public.documents set document_type='scope-of-work',updated_at=now() where document_type='scope_of_work' and status::text<>'superseded';

update public.documents legacy
set status='superseded'::public.document_status,is_current_revision=false,superseded_at=coalesce(legacy.superseded_at,now()),updated_at=now()
where legacy.document_type='client_quote' and exists(
  select 1 from public.documents canonical
  where canonical.organisation_id=legacy.organisation_id and canonical.document_type='client-quote'
    and canonical.status::text<>'superseded' and canonical.quote_id=legacy.quote_id
);
update public.documents set document_type='client-quote',updated_at=now() where document_type='client_quote' and status::text<>'superseded';

update public.documents
set status='superseded'::public.document_status,is_current_revision=false,superseded_at=coalesce(superseded_at,now()),updated_at=now()
where document_type='project_closeout' and status::text='draft';
update public.documents set document_type='completion-report',updated_at=now() where document_type='project_closeout' and status::text<>'superseded';

with ranked as (
  select id,row_number() over(
    partition by organisation_id,quote_id
    order by case status::text when 'published' then 6 when 'issued' then 5 when 'approved' then 4 when 'signed' then 3 when 'in_review' then 2 else 1 end desc,created_at desc
  ) rn
  from public.documents
  where document_type='client-quote' and quote_id is not null and status::text<>'superseded'
)
update public.documents d
set status='superseded'::public.document_status,is_current_revision=false,superseded_at=coalesce(d.superseded_at,now()),updated_at=now()
from ranked r where d.id=r.id and r.rn>1;

-- ------------------------------------------------------------------
-- Acquisition is the sole pre-commercial Partner Review owner
-- ------------------------------------------------------------------
...
create trigger trg_op_guard_case_partner_review_creation before insert on public.partner_review_requests
for each row execute function public.op_guard_case_partner_review_creation();

-- ------------------------------------------------------------------
-- Prospect -> Case uses the governed Acquisition evidence only
-- ------------------------------------------------------------------
create or replace function public.op_convert_prospect(p_organisation_id uuid,p_user_id uuid,p_prospect_id uuid)
...
declare
  ...
  v_review_request_id uuid; v_partner_id uuid; v_lead_ref text; v_doc_ref text;
begin
  ...
  order by d.created_at desc,q.created_at desc limit 1;
  if v_review_request_id is null then raise exception 'OS_INTEGRITY: Case 360 requires approved Acquisition Partner Review, positive Partner pricing and Go / No-Go approval.'; end if;
  ...
  if not found then raise exception 'Submitted Step 2 technical evidence could not be found.'; end if;
  ...
  if not found then
    ...
    values(p_organisation_id,v_prospect.company_name,v_prospect.industry,p_user_id) returning * into v_company;
  end if;
  ...
  insert into public.leads(
    ...
    'qualified',v_prospect.requirement_summary,v_company.id,case when v_contact.id is null then null else v_contact.id end,v_prospect.id,
    v_prospect.source,v_prospect.company_name||' — '||v_prospect.project_type,'Engineering overflow support','normal',v_lead_ref,
  ...
  values(p_organisation_id,v_lead.id,v_intake.id,p_user_id,'client-requirements',v_doc_ref,'Client Requirements','draft',1);

  -- Existing inheritance trigger transfers approved Partner Review / pricing into Case ownership.
  ...
  perform public.op_record_activity(p_organisation_id,p_user_id,'prospect',v_prospect.id,'prospect_converted',jsonb_build_object('leadId',v_lead.id,'leadReference',v_lead.reference,'partnerReviewRequestId',v_review_request_id,'partnerId',v_partner_id));
  perform public.op_record_activity(p_organisation_id,p_user_id,'lead',v_lead.id,'case_created_from_governed_acquisition',jsonb_build_object('prospectId',v_prospect.id,'reference',v_lead.reference,'partnerReviewRequestId',v_review_request_id));
  return v_lead;
end $$;

-- ------------------------------------------------------------------
-- Case technical basis and commercial progression
-- ------------------------------------------------------------------
create or replace function public.op_approve_technical_intake(p_organisation_id uuid,p_user_id uuid,p_intake_id uuid)
returns public.technical_intakes
language plpgsql security definer set search_path=public
as $$
declare v_intake public.technical_intakes; v_ref text;
begin
  perform public.op_assert_membership(p_organisation_id,p_user_id);
  select * into v_intake from public.technical_intakes where organisation_id=p_organisation_id and id=p_intake_id for update;
  ...
  if not exists(select 1 from public.documents d where d.organisation_id=p_organisation_id and d.lead_id=v_intake.lead_id and d.document_type='client-requirements' and d.status::text in ('approved','issued','published') and d.is_current_revision=true) then
    raise exception 'Approved Client Requirements are required before Technical Scope approval.';
  end if;
  if not exists(select 1 from public.partner_review_requests r where r.organisation_id=p_organisation_id and r.lead_id=v_intake.lead_id and r.status::text in ('approved','approved_with_conditions')) then
    raise exception 'OS_INTEGRITY: Case 360 is missing the approved Partner Review inherited from Acquisition.';
  end if;

  update public.technical_intakes set status='approved',reviewed_by=p_user_id,reviewed_at=now(),updated_at=now() where id=p_intake_id returning * into v_intake;
  update public.leads set status='qualified',next_action='Approve controlled Scope of Work and Partner Technical Assessment',updated_at=now() where organisation_id=p_organisation_id and id=v_intake.lead_id;
  ...
  perform public.op_record_activity(p_organisation_id,p_user_id,'technical_intake',v_intake.id,'technical_intake_approved',jsonb_build_object('leadId',v_intake.lead_id,'canonicalLifecycle',true));
  return v_intake;
end $$;

create or replace function public.op_create_commercial_review(p_organisation_id uuid,p_user_id uuid,p_partner_quote_id uuid,p_markup_percent numeric)
returns public.commercial_reviews
language plpgsql security definer set search_path=public
as $$
declare
  v_quote public.partner_quotes; v_request public.partner_review_requests; v_response public.partner_review_responses;
  v_decision public.partner_review_internal_decisions; v_review public.commercial_reviews;
  v_cost numeric; v_client_price numeric; v_margin numeric; v_margin_percent numeric;
begin
  perform public.op_assert_membership(p_organisation_id,p_user_id);
  ...
  v_cost:=v_quote.price; v_client_price:=round(v_cost*(1+p_markup_percent/100.0),2); v_margin:=round(v_client_price-v_cost,2);
  v_margin_percent:=case when v_client_price>0 then round((v_margin/v_client_price)*100.0,4) else 0 end;
  insert into public.commercial_reviews(organisation_id,lead_id,partner_quote_id,cost_price,client_price,margin_amount,margin_percent,status,created_by)
  values(p_organisation_id,v_quote.lead_id,v_quote.id,v_cost,v_client_price,v_margin,v_margin_percent,'pending_approval',p_user_id) returning * into v_review;
  ...
  perform public.op_record_activity(p_organisation_id,p_user_id,'lead',v_quote.lead_id,'commercial_margin_created',jsonb_build_object('commercialReviewId',v_review.id,'partnerQuoteId',v_quote.id,'costPrice',v_cost,'markupPercent',p_markup_percent,'clientPrice',v_client_price,'canonicalLifecycle',true));
  return v_review;
end $$;

create or replace function public.op_approve_commercial_generate_quote(p_organisation_id uuid,p_user_id uuid,p_review_id uuid,p_currency text default 'GBP',p_vat_rate numeric default 20)
...
begin
  ...
  v_number:=public.op_next_reference(p_organisation_id,'quote',1); v_vat:=round((v_review.client_price*p_vat_rate/100.0)::numeric,2);
  ...
  values(p_organisation_id,p_user_id,v_review.lead_id,v_review.id,v_number,1,'draft',v_review.client_price,v_vat,v_review.client_price+v_vat,upper(p_currency)) returning * into v_quote;
  ...
  perform public.op_record_activity(p_organisation_id,p_user_id,'quote',v_quote.id,'client_quote_generated',jsonb_build_object('commercialReviewId',p_review_id,'vatRate',p_vat_rate,'documentType','client-quote','canonicalLifecycle',true));
  return v_quote;
end $$;

-- ------------------------------------------------------------------
-- Written client acceptance backs Project creation
-- ------------------------------------------------------------------
create or replace function public.op_guard_project_client_acceptance()
returns trigger language plpgsql set search_path=public as $$
begin
  if new.quote_id is not null and not exists(select 1 from public.quote_acceptance_records a where a.organisation_id=new.organisation_id and a.quote_id=new.quote_id) then
...
create trigger trg_op_guard_project_client_acceptance before insert on public.projects for each row execute function public.op_guard_project_client_acceptance();

create or replace function public.op_accept_quote_create_project_with_acceptance(
  p_organisation_id uuid,p_user_id uuid,p_quote_id uuid,p_acceptance_basis text,p_evidence_reference text,
  p_accepted_by_name text,p_accepted_by_email text default null,p_notes text default null
) returns public.projects
language plpgsql security definer set search_path=public
as $$
declare v_quote public.quotes; v_lead public.leads; v_project public.projects; v_number text; v_stw_ref text;
begin
  perform public.op_assert_membership(p_organisation_id,p_user_id);
  ...
  if not exists(select 1 from public.documents where organisation_id=p_organisation_id and quote_id=p_quote_id and document_type='client-quote' and status::text in ('issued','published') and is_current_revision=true) then
    raise exception 'Controlled Client Quote must be issued before acceptance can create a Project.';
  end if;
  ...
  values(p_organisation_id,p_user_id,p_user_id,v_quote.lead_id,v_quote.id,v_number,coalesce(nullif(v_lead.title,''),v_lead.company_name||' engineering project'),'planning',current_date,v_lead.notes,'mobilisation')
  ...
  perform public.op_record_activity(p_organisation_id,p_user_id,'project',v_project.id,'project_created_from_governed_client_acceptance',jsonb_build_object('quoteId',p_quote_id,'acceptanceBasis',p_acceptance_basis,'evidenceReference',trim(p_evidence_reference),'canonicalLifecycle',true));
  return v_project;
end $$;

create or replace function public.op_accept_quote_create_project(p_organisation_id uuid,p_user_id uuid,p_quote_id uuid)
returns public.projects language plpgsql security definer set search_path=public as $$
begin raise exception 'OS_INTEGRITY: Use governed client acceptance evidence before creating Project 360.'; end $$;
...
-- ------------------------------------------------------------------
-- Execution Partner = commercially approved Partner + controlled scope
-- ------------------------------------------------------------------
create or replace function public.op_guard_project_execution_assignment()
returns trigger language plpgsql set search_path=public as $$
declare v_commercial_partner_id uuid; v_partner public.partners; v_doc public.documents;
begin
  select pq.partner_id into v_commercial_partner_id
  from public.projects p join public.quotes q on q.id=p.quote_id
...
create trigger trg_op_guard_project_execution_assignment before insert or update of project_id,partner_id,scope_document_id on public.project_execution_assignments
...
returns trigger language plpgsql set search_path=public as $$
declare v_assignment public.project_execution_assignments; v_stage text;
begin
...
create trigger trg_op_guard_partner_delivery_cycle before insert on public.partner_delivery_submissions for each row execute function public.op_guard_partner_delivery_cycle();

-- ------------------------------------------------------------------
-- Canonical Project stage readiness core. Later migrations add physical
-- Partner files and execution-cycle-specific client evidence around this core.
-- ------------------------------------------------------------------
create or replace function public.op_project_stage_readiness(p_project_id uuid)
returns jsonb language plpgsql stable security definer set search_path=public
as $$
declare
  p public.projects; v_stage text; v_reasons jsonb:='[]'::jsonb;
  v_task_total integer:=0; v_task_open integer:=0;
  v_delivery_total integer:=0; v_delivery_work_open integer:=0; v_delivery_close_open integer:=0;
  v_delivery_internal_block integer:=0; v_delivery_client_block integer:=0; v_open_exceptions integer:=0;
  v_assignment_id uuid; v_assignment_partner_id uuid; v_execution_cycle integer:=1; v_partner_commenced boolean:=false;
  v_commercial_partner_id uuid; v_current_submission_id uuid; v_current_submission_review text;
  v_scope_ready boolean:=false; v_statement_ready boolean:=false; v_technical_review_ready boolean:=false;
  v_register_ready boolean:=false; v_handover_issued boolean:=false; v_completion_issued boolean:=false; v_financial jsonb;
  v_open_receivables integer:=0; v_open_payables integer:=0;
begin
  select * into p from public.projects where id=p_project_id;
  if not found then return jsonb_build_object('ready',false,'stage','unknown','reasons',jsonb_build_array('Project not found.')); end if;
  ...
  select count(*),count(*) filter(where status in ('not_started','in_progress','blocked')),
  ...
  select pq.partner_id into v_commercial_partner_id
  from public.quotes q join public.commercial_reviews cr on cr.id=q.commercial_review_id join public.partner_quotes pq on pq.id=cr.partner_quote_id
  ...
  if v_assignment_id is not null then
    ...
    from public.partner_delivery_submissions s where s.assignment_id=v_assignment_id and s.execution_cycle=v_execution_cycle
  ...
  if v_stage='mobilisation' then
    ...
    if v_delivery_work_open>0 then v_reasons:=v_reasons||jsonb_build_array(v_delivery_work_open||' Delivery Control item(s) are still active or blocked.'); end if;
    if v_assignment_id is not null then
      if v_current_submission_id is null or coalesce(v_current_submission_review,'') not in ('submitted','under_review','accepted') then v_reasons:=v_reasons||jsonb_build_array('A current-cycle Execution Partner delivery submission is required before Internal Review.'); end if;
    else
  ...
  return jsonb_build_object(
    'ready',jsonb_array_length(v_reasons)=0,'stage',v_stage,'reasons',v_reasons,
    ...
    'openPartnerExceptions',v_open_exceptions,'executionMode',case when v_assignment_id is null then 'internal' else 'partner' end,
